...
class PublishCreateController extends CommonController {
    ...
    */
    ...
    public function index() {
        ...
        $class_id = I('class_id');
        if($class_id == 10) $this->assign('isCheckOn',M('Config')->where(['sign'=>'dujiaCheck'])->getField('config'));
        if(!empty($class_id)){
            //获取问题分类
        ...
        if(I('is_home') == 1) $map['p.is_home'] = "1";
        //手动获取列表
    ...
    */
    public function save(){
		...
		if (empty($id)) {
            ...
            if (!empty($_FILES['fileInput']['tmp_name'])) {
                $return = D('Uploads')->uploadImg("fileInput", "publish", $rs,'',"[[200,200,{$rs}]]");
                if($return['status'] == 1)
            ...
            if (!empty($_FILES['fileInput']['tmp_name'])) {
                ...
                //上传图片
                $return = D('Uploads')->uploadImg("fileInput", "publish", $id,'',"[[200,200,{$id}]]");
                //修改路径
                if($return['status'] == 1)
