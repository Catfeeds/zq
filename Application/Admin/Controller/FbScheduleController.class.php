 ...
 * @since  2015-12-21
 */
class FbScheduleController extends CommonController {
    public $gameType;
    /**
     *构造函数
     *
    ...
    public function index() {
        ...
        $list = $this->_list(D("gameFbinfo"), $map, $desc);

//        $count =  M('gameFbinfo')->where($map)->count();
//        $list = M('gameFbinfo')
//            ->where($map)
//            ->field("id,game_id,union_name,union_id,is_gamble,is_show,status,web_video,is_video,show_date,is_gamble,is_betting,web_video")
//            ->order($desc)
//            ->limit($pageNum)
...
//            ->select();
        ...
        foreach ($gameIds as $gk => $gv){
            $gameIdArr[] = (int)$gv;
        }

        if($gameIdArr){
            $_mGameList = $mongo->select(
                'fb_game',
                ['game_id' => ['$in' => $gameIdArr]],
                ['game_id', 'union_id', 'gtime', 'game_start_timestamp','home_team_id', 'away_team_id',
                    'home_team_name', 'away_team_name','score', 'half_score', 'match_odds', 'is_go', 'game_state',
                    'score', 'half_score','match_odds.3'
                ]
            );
            foreach ($_mGameList as $mgk => $mgv){
                $mGameList[$mgv['game_id']] = $mgv;
            }
        }

        //联赛信息
        $unionIds = array_unique(array_column($mGameList, 'union_id'));

        if($unionIds){
            $_unions = $mongo->select(
                'fb_union',
                ['union_id' => ['$in' => array_values($unionIds)]],
                ['union_id', 'union_name', 'level']
            );

            foreach ($_unions as $uk => $uv){
                $unions[$uv['union_id']] = $uv;
            }
        }

        foreach($list as $k => $v){
            $mGame = $mGameList[$v['game_id']];
            $union = $unions[$mGame['union_id']];
            $odds = $mGame['match_odds'][3];

            $list[$k]['union_name'] = $union['union_name'][0] ?: explode(',', $v['union_name'])[0];
            $list[$k]['home_team_name'] = $mGame['home_team_name'][0];
            $list[$k]['away_team_name'] = $mGame['away_team_name'][0];
            $list[$k]['level'] = $union['level'];
            $list[$k]['game_state'] = $mGame['game_state'];
            $list[$k]['score'] = $mGame['score'];
            $list[$k]['half_score'] = $mGame['half_score'];
            $list[$k]['fsw_exp'] = $odds[1];
            $list[$k]['fsw_ball'] = $odds[13];
            $list[$k]['is_go'] = (int)$mGame['is_go'];
            $list[$k]['is_gamble'] = $odds[1] != ' ' && $odds[13] != ' ' && (int)$v['is_gamble'] ? 1 : 0;

            $list[$k]['gtime'] = $mGame['game_start_timestamp'] ? date('Y-m-d H:i', $mGame['game_start_timestamp']) : $mGame['gtime'];
        }
