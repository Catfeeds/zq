...
class IntroController extends CommonController
{
    ...
    public function index()
    {
        ...
        if (!$intro_class) {
            $list = M('IntroClass')->where(['status'=>1])->order('sort')->limit(4)->select();
            foreach ($list as &$val) {
...
//        if ($res) return true;
     ...
     * 球王详情页
     */
    public function intro_info($path)
    {
        C('HTTP_CACHE_CONTROL','no-cache,no-store');
        //$products_id = I('id');
        $products_id = M("IntroProducts")->where(['path' => $path])->getField('id');
        ...
        //seo
        $seo = [
            'seo_title' => $products_info['seo_title'] ?: $products_info['name'].'|球王|足彩分析|足球分析|足球推荐_全球足球赛事亚盘推荐分析_全球体育网',
            'seo_keys'  => $products_info['seo_keys'] ?: $products_info['name'].'|球王|足彩分析|足球分析|足球推荐_全球足球赛事亚盘推荐分析_全球体育网',
            'seo_desc'  => $products_info['seo_desc'] ?: $products_info['desc'].'|球王|足彩分析|足球分析|足球推荐_全球足球赛事亚盘推荐分析_全球体育网',
        ];
        $this->setSeo($seo);
...
//        $map['pub_time'] = array('lt',$time);
        ...
        //设置分页路由链
        $page->url = "/qiuwang/".$path.".html?p=%5BPAGE%5D";
        ...
        $this->display('Intro/intro_info');
    }

    /*
     * ajax执行用户订阅取消操作
     */
    public function user_follow($productId)
    {
     ...
     * ajax获取历史曲线数据
     */
    public function history_tab($key)
    {
        $today = mktime(0, 0, 0, date('m'), date('d'), date('Y'));
