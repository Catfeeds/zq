...
class WebfbController extends Controller
{
    ...
    {
        $companyID = I('cid',3,'intval');
        $gameId    = I('gameId',0,'intval');
        $webfbService = new \Home\Services\WebfbService();
        $res = $webfbService->getOddsData($companyID,$gameId);   //数据库取数据
    ...
    public function getFbPanlu()
    {
        //半场的前6场的历史战绩
        $webfbService = new \Home\Services\WebfbService();
        $gameId = I('gameId','','intval');
        $panlu = $webfbService->getPanluWeb(10,$gameId);   //数据库取值
     ...
     * 篮球赛事积分等排行榜
     * @return json
     * @author Hmg 2018-01-22
     */
    public function bkUnionRank()
    {
        $unionId = $_REQUEST['unionId'];
        $type = $_REQUEST['type'];
        $num = !empty($_REQUEST['num'])?$_REQUEST['num']:'10';

        $webfbService = new \Home\Services\WebfbService();
        $res = $webfbService->getBkUnionRank($unionId,$type,$num);   //数据库取值
     ...
     * @author Hmg 2018-01-22
     */
    public function fbUnionRank()
    {
        $unionId = $_REQUEST['unionId'];
        $num = !empty($_REQUEST['num'])?$_REQUEST['num']:'10';

        $webfbService = new \Home\Services\WebfbService();
        $res = $webfbService->getFbUnionRank($unionId,$num);   //数据库取值
     ...
     * @author Hmg 2018-01-22
     */
    public function fbUnionArcher()
    {
        $unionId = $_REQUEST['unionId'];
        $num = !empty($_REQUEST['num'])?$_REQUEST['num']:'10';

        $webfbService = new \Home\Services\WebfbService();
        $res = $webfbService->getFbUnionArcher($unionId,$num);   //数据库取值
