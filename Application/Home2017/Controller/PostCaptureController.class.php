...
//        var_dump($all);
    ...
    function getPic(){
        ...
        $urlArr = getWebConfig('urlArr1');
        if(empty($urlArr)){
            $urlArr = [];
            $rules = [
                'link' => array('.page a', 'href'),
            ];
            ...
            M('Config')->where(['sign' => 'urlArr1'])->save(['config' => json_encode($urlArr)]);
        }

//var_dump($urlArr);die;
        $usedPage = getWebConfig('usedPage');
/*
        $urlArr = [
            ["http://www.mm131.com/xinggan/list_6_8.html", "http://www.mm131.com/xinggan/list_6_6.html"],
            ["http://www.mm131.com/qingchun/list_1_2.html", "http://www.mm131.com/qingchun/list_1_3.html"],
        ];
*/
        $model = D('Gallery');
...
//                $v1 = 'http://www.mm131.com/xinggan/list_6_8.html';

                $data = QueryList::Query($v1, $rules, $range)
                    ->getData(function ($item) use ($class_id, $model, $dataArr) {
                        //避免相同不同页数引起的链接一致
                        if(!in_array($item['link'], $dataArr)) {
                            $data = [];
...
//                            $picName = str_replace('0.jpg', '', $item['img']);
                            $picName = substr($item['img'], 0, strripos($item['img'], '/')+1);
                            $picArr = [];
                            for ($i = 1; $i < 31; $i++) {
                                $picUrl = "{$picName}{$i}.jpg";

                                $img = Tool::url_get_contents($picUrl);
//                                $img = Tool::GrabImage($picUrl);
                                //直接请求，判断404
                                if (stripos($img, 'Not Found')) {
                                    break;
                                } else {
        ...
        M('Config')->where(['sign' => 'usedPage'])->save(['config' => json_encode($usedPage)]);
