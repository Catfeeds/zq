...
class CommonController extends Controller {
    ...
    */
    ...
    public function getRecommendJJ($number=3)
    {
        ...
        {
            $prefix = C('IMG_SERVER');
            $highlights = M('Highlights')->where(['is_recommend'=>2,'status'=>1])->order("add_time desc")->field("id,game_id,game_type,title,concat('$prefix',img) img,web_url,web_ischain")->limit($number)->select();
    ...
    public function ajaxCheckLogin()
    {
        ...
        $userid = is_login();
        if($userid){
            $userArr = M('FrontUser')->master(true)->where(['id'=>$userid])->field('id,head,coin+unable_coin as balance,status,nick_name,password,session_id')->find();
            //是否在别处登录
