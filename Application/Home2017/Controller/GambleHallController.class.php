    ...
    */
    ...
    public function myGamble()
    {
        ...
        foreach ($history as $k => $v) {
            //获取被查看推荐的数量
            $history[$k]['logCount'] = M('quizLog')->where(array('gamble_id'=>$v['id']))->count();
            if($gamble_type == 2){
                $history[$k]['bet_code'] = M('fbBetodds')->where(array('game_id'=>$v['game_id']))->getField('bet_code');
            }
        }
