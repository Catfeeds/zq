...
class AppdataService
{
    ...
    public function getDetailAppB($gameId)
    {
        $gameId = (int) $gameId;
        if(empty($gameId)) return false;
        $res = M()->query('select * from qc_detail_fb where game_id='.$gameId.' order by gtime');
    ...
    public function getLineup($gameId)
    {
        ...
        if(!empty($res))
        {
            $defaultHomeImg = SITE_URL.$_SERVER['HTTP_HOST'].'/Public/Home/images/common/fb_ht.png';
            $defaultAwayImg = SITE_URL.$_SERVER['HTTP_HOST'].'/Public/Home/images/common/fb_at.png';
    ...
    public function getContinent()
    {
        ...
        $res = $continent->field('continent_id,continent_name')->select();
        ...
        if(!empty($res))
        {
            ...
            foreach($res as $k=>$v)
            {
                $temp = array($v['continent_id'],$v['continent_name']);
                $aData[] = $temp;
            }
            return $aData;
        }
    ...
    public function getCupMatch($unionId,$runId = '')
    {
        ...
        else
        {
            if(empty($runId))
            {
                ...
                $rData = array('info'=>$info,'group'=>$res['group'], 'content'=>array('runNo' =>$last, 'content' =>$res['intContent'][$last[0]]));
            }
            else
            {
                $runNo = [];
     ...
     */
    public function getTeamLogo($gameId)
    {
        if(empty($gameId))
            return false;

        $baseRes = M('GameFbinfo f')
            ->field('f.game_id,f.gtime,f.game_state,f.score,f.union_name,f.home_team_name,f.away_team_name,b.img_url as home_img_url,c.img_url as away_img_url,is_video,is_flash,gtime,f.home_team_id,f.away_team_id,f.union_id,is_betting,u.is_union,f.is_go,f.weather,f.game_half_time')
        ...
        if(!empty($baseRes))
        {
            ...
            $aData[0] = $homeTeamImg;
            $aData[1] = $awayTeamImg;
            $aData[2] = (string) $baseRes['game_state'];
            $aData[3] = (string) $baseRes['score'];
            $aData[4] = date('Ymd',$baseRes['gtime']);
            $aData[5] = date('H:i',$baseRes['gtime']);
            $aData[6] = explode(',', $baseRes['union_name']);
            $aData[7] = explode(',', $baseRes['home_team_name']);
            $aData[8] = explode(',', $baseRes['away_team_name']);
            if($baseRes['game_state'] == -1)
                $aData[9] = '0';
            else
                $aData[9] = $baseRes['is_video'];
            #是否flash
            $bmap['game_id']  = $gameId;
            ...
            $aData[11] = $baseRes['gtime'];
            $aData[12] = $baseRes['home_team_id'];
            $aData[13] = $baseRes['away_team_id'];
            $aData[14] = $baseRes['union_id'];
            $aData[15] = $baseRes['is_betting'];
            $aData[16] = $baseRes['is_union'] === null?'':$baseRes['is_union'];

            //关注(userToken,设备标识deviceID，赛事类型gameType，赛事ID gameId)
            $redis  = connRedis();
            $uinfo  = getUserToken(I('userToken'));
            ...
            $aData[17] = $is_follow === true || $is_follow2 === true ? '1': '0';//1关注，0未关注

            $aData[18] = $baseRes['is_go'] === null?'':$baseRes['is_go'];
            //var_dump($baseRes);exit;
            $baseRes['weather'] = str_replace(['&nbsp;',' '],'',$baseRes['weather']);
            $wTemp = explode('℃',$baseRes['weather']);
            $aData[19] = ($baseRes['weather'] === null || $baseRes['weather'] === '')?'':$wTemp[1].$wTemp[0].'℃';
            if(!empty($baseRes['game_half_time']))
            {
                $tempTime = explode(',',$baseRes['game_half_time']);
                $tempTime[1] = str_pad($tempTime[1]+1,2,0,STR_PAD_LEFT);
                $tempTime[2] = str_pad($tempTime[2],2,0,STR_PAD_LEFT);
                $aData[20] = implode('',$tempTime);
            }
            else
            {
                $aData[20] = '';
