...
*/
               ...
               /* $val[26] = !empty($v['fsw_exp_home']) ? $v['fsw_exp_home'] : '';
                ...
                if($v['game_state'] != -1)
                {
                    $val[42] = (string)$v['is_video'];      //是否有直播
                    //$val[43] = isset($blData[$v['game_id']])?(string)$blData[$v['game_id']]:'0';    //是否有动画
       ...
       // echo M()->getlastsql();exit;
        if(!empty($GameList))
        {
            ...
            foreach ($GameList as $k => $v)
            {
                ...
                if($v['game_state'] != -1)
                {
                    $val[26] = (string)$v['is_video'];      //是否有直播
                    $val[27] = isset($blData[$v['game_id']])?(string)$blData[$v['game_id']]:'0';    //是否有动画
                }
                else
                {
                    $val[26] = '0';
    ...
    public function getTeamLogo($gameId)
    {
        ...
        $baseRes = M('GameBkinfo f')
            ->field('f.game_id,f.gtime,f.game_state,f.score,f.union_name,f.home_team_name,f.away_team_name,b.img_url as home_img_url,c.img_url as away_img_url,is_video,is_flash,gtime,home_team_id,away_team_id,f.union_id,f.list_score,f.is_ot,f.ot_score')
            ->join('LEFT JOIN qc_game_teambk b ON f.home_team_id = b.team_id LEFT JOIN qc_game_teambk c ON f.away_team_id = c.team_id')
        ...
        if(!empty($baseRes))
        {
            ...
            {
                if(in_array($baseRes['game_state'],[1,2,3,4,5,6,7,50]))
                {
            ...
            //$aData[10] = !empty($flashRes)?(string)$flashRes['is_link']:'0';
            ...
            $aData[12] = $is_follow === true || $is_follow2 === true ? '1': '0';//1关注，0未关注
            $aData[13] = $baseRes['union_id'];
            $aData[14] = $baseRes['home_team_id'];
            $aData[15] = $baseRes['away_team_id'];
            //$aData[16] = $baseRes['list_score'];
            if(empty($baseRes['ot_score']))
                $aData[16] = $baseRes['list_score'];
            else
                 $aData[16] = $baseRes['list_score'].','.$baseRes['ot_score'];
        }
