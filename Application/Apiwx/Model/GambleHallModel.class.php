...
class GambleHallModel extends Model
{
    ...
    public function gamble($userid,$param,$platform,$gameType=1)
    {
        ...
        //是否有推荐购买和推荐分析
        $userInfo = M('FrontUser')->field([$Lv,'point','nick_name'])->where(['id'=>$userid])->find();
        ...
        foreach ($sub_users as $k => $sub_id) {
            ...
            if($ApnsUser &&  $ApnsUser['feed_back'] != '1'){
    ...
    * @return  array
    */
    public function getWinning($id,$gameType=1,$playType=0,$gambleType=1,$limit = null)
    {
        $GambleModel = $gameType == 1 ? M('Gamble') : M('Gamblebk');
    ...
    public function getIndexUser(){
        ...
        foreach ($res as $k => $v) {
            ...
            $res[$k]['tenGambleRate'] = $v['gameType'] == 1 ? (string)($userGamble[3][$v['user_id']]/10) : (string)($userBetting[3][$v['user_id']]/10);//命中率
    ...
    public function masterGamble(){
        ...
        foreach($totalArr as $k => $v){
            ...
            $tenGamble = D('GambleHall')->getTenGamble($v['user_id'], 1, $gambleType);
            $winnig    = D('GambleHall')->getWinning($v['user_id'], 1, 0, $gambleType); //连胜记录

            $totalArr[$k]['tenGambleRate']  = countTenGambleRate($tenGamble, $gambleType)/10;//近十场的胜率;
    ...
    public function superMasterData($playType, $limit, $beginTime, $endTime, $lastUser=''){
        ...
        if($gambleList){
            ...
            foreach ($res as $k => $v) {
                ...
                $winnig = D('GambleHall')->getWinning($v['user_id'], 1, 0, $playType); //连胜记录
                ...
                unset($tenGamble, $winnig, $res[$k]['gamble_num'], $res[$k]['bet_num']);
            }
        }
