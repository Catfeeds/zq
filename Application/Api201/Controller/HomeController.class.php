...
class HomeController extends PublicController
{
    ...
    public function index()
    {
        ...
        //直播和集锦
        if(!$video = S('HomeVideo'.MODULE_NAME)){
            $video = D('Home')->getVideo();
            S('HomeVideo'.MODULE_NAME, json_encode($video), 60*5);
        }
        ...
        //热门竞猜
        if(!$match = S('HomeMatch'.MODULE_NAME)){
            $match = D('Home')->getMatch();
            S('HomeMatch'.MODULE_NAME, json_encode($match), 60*2);
        }

        //名师解盘
        $publishOrder = 'is_channel_push desc,add_time desc,id desc'; //资讯排序

        if(!$famous = S('HomeFamous'.MODULE_NAME)){
            ...
            S('HomeFamous'.MODULE_NAME, json_encode($famous), 60*5);
        }

        //名师解盘轮播
        if(!$famousArticle = S('HomeFamousArticle'.MODULE_NAME)){
            $famousArticle = M('PublishList')->field(['id','title'])->where(['class_id'=>10,'status'=>1])->order($publishOrder)->limit(10)->select();
            S('HomeFamousArticle'.MODULE_NAME, json_encode($famousArticle), 60*5);
        }

        //资讯快报轮播
        if(!$newsArticle = S('HomeNewsArticle'.MODULE_NAME)){
            $newsArticle = M('PublishList')->field(['id','title']) ->where(['class_id'=>['neq',10],'status'=>1])->order($publishOrder)->limit(10)->select();
            S('HomeNewsArticle'.MODULE_NAME, json_encode($newsArticle), 20);
        }

        //资讯快报列表
        if(!$articleList = S('HomeArticleList'.MODULE_NAME)){
            $articleList = D('Home')->getArticleList(1,10);
            S('HomeArticleList'.MODULE_NAME, json_encode($articleList), 20);
        }
