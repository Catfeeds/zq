...
class EasemobController extends PublicController
{
    ...
    */
    ...
    public function sendText()
    {
        ...
        //1：资讯  2：图集  9：外链
        $content = '女子链球张文秀获银牌 波兰神人破世界纪录夺冠';
        $ext = [
            'em_apns_ext' => ['em_push_title'=>$content,'em_module'=>['module'=>9,'url'=>'http://m.qqty.com']]
        ];

        $info = $Easemob->sendText($from="admin",$target_type='users',$target=[$this->param['username']],$content,$ext);
        $this->ajaxReturn(['info'=>$info]);
    }
}
