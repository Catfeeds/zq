        ...
        if(!$userArr = S('userArr'.$sortType.MODULE_NAME)) {
            ...
            foreach ($userArr as $k => $v) {
                if ($sortType == 'highHit') {
                    ...
                    $rateNum = M('QuizLog')->where(['cover_id' => $v, 'game_type' => 1, 'log_time' => ['between', [strtotime(date('Y-m-d 00:00:00', strtotime('-3 day'))), strtotime(date('Y-m-d 23:59:59', strtotime('-1 day')))]]])->count();
                    //销量为0去掉
                    if($rateNum < 1){
                        unset($userArr[$k], $arr[$v]);
                        continue;
