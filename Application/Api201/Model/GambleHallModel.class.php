    ...
    */
    public function getRankingData($gameType=1,$dateType=1,$user_id=null,$more=false,$page='',$pageNum='',$todayGamble=false)
    {
        ...
        if ($page && $pageNum) //是否分页
        {
            ...
            {
                $Ranking = M('rankingList r')->join('left join qc_front_user f on f.id = r.user_id')->field($field)->where($where)->order("ranking asc")->page($page.','.$pageNum)->select();
            }
        }
        else
        {
            $Ranking = M('rankingList r')->join('left join qc_front_user f on f.id = r.user_id')->field($field)->where($where)->order("ranking asc")->select();
        }
    ...
    public function getGambleList($userid,$playType=null,$page=null,$gamble_id=0)
    {
        ...
        if ($page)
        {
            if($gamble_id){
                ...
                $list = M('Gamble g')->join($joinUnion)->join($joinGameInfo)->field($field)->where($where)->limit($pageNum)->order($orderBy)->group('g.id')->select();
            }else{
                $list = M('Gamble g')->join($joinUnion)->join($joinGameInfo)->field($field)->where($where)->page($page.','.$pageNum)->order($orderBy)->group('g.id')->select();
            }
        }
        else
        {
            $list = M('Gamble g')->join($joinUnion)->join($joinGameInfo)->field($field)->where($where)->order($orderBy)->group('g.id')->limit(10)->select();
        }
