...
/**
 +------------------------------------------------------------------------------
 * MongoService   mongodb服务类
 +------------------------------------------------------------------------------
 * Copyright (c) 2017 http://www.qqw.com All rights reserved.
 +------------------------------------------------------------------------------
...
class MongodbService
{
     ...
     *
     * 参数：array('username' => new mongoRegex("/yhp.*\/")) 前面斜杠不用要
    ...
    {
        $DBconfig = !$DBconfig? C('DB_MONGO') : $DBconfig;
        ...
        $this->selectDB($DBconfig['DB_NAME']);
        // 命令前缀
        if(!isset($config['cmd'])){
    ...
    public function aggregate($colName, $ops){
        $db = $this->_db;
        $table = $db->selectCollection($colName);
        $result = $table->aggregate($ops);
    ...
    private function _getCol($colName){
        $col = $this->_db->selectCollection($colName);
        if($this->index){
            $col->ensureIndex($this->index);
        }
        return $col;
    }
