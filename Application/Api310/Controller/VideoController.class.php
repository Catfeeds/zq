...
//            $list[$k]['awayTeamLogo']   = getLogoTeam($v['away_team_id'],2,$game_type);
    ...
    public function articleList()
    {
        ...
        if(!$responseList = S($cacheKey))
        {
            ...
            $articleList = (array)M('PublishList')->field(['id', 'title', 'remark', 'img', 'content', 'add_time'])
                ->where([$akey => $game_id, 'status' => 1])
                ->order('is_recommend desc, is_channel_push desc, add_time desc')
                ->limit(20)
                ->select();

            $videoList = (array)M('Highlights')->field(['id', 'title', 'remark', 'img', 'app_url', 'app_ischain', 'is_prospect', 'add_time','app_isbrowser'])
...
//            if ($responseList)
//                S($cacheKey, $responseList, 60 * 30);
        }

        $lists = D('Home')->getArticleImg($responseList, false);
        $this->ajaxReturn(['articleList' => $lists ?:[]]);
    }
