...
//            unset($adver[$k]['id']);
    ...
    public function gamblePage ()
    {
        ...
        if($gameType ==  '1'){
            ...
            $userGamble = (array)M('Gamble')->field(['play_type', 'chose_side'])->where(['game_id' => $gameId, 'user_id' => $userToken['userid']])->select();
        ...
        elseif($gameType == '2')
        {
            ...
            $userGamble = (array)M('Gamblebk')->field(['play_type', 'chose_side'])->where(['game_id' => $gameId, 'user_id' => $userToken['userid']])->select();
        }
        ...
        //今日剩余推荐场数，历史的推荐就不返回了
        if(is_array($userToken) && $gstate_check == 1){
    ...
    public function gambleCount()
    {
        ...
        if($list){
            ...
            foreach ($pageList as $k => $v)
            {
                ...
                $pageList[$k]['quiz_number'] = D('Common')->getQuizNumber($v['quiz_number'],$gameType);
            }
        }
    ...
    public function userHomePage()
    {
        ...
        if($page == 1){
            $userInfo                = M('FrontUser')->field(['nick_name','lv','lv_bet','lv_bk','descript','head face'])->where(['id'=>$user_id])->find();
...
//                        ->select();
                    ...
                    foreach($tempRank as $k2=>$v2){
                        if(in_array_case($v2['user_id'], $gambleUsers)){
                            $rank1[] = $v2;
                        }
                    }
...
//                    ->select();
    ...
    public function masterGamble(){
        ...
        $list = D('Home')->getMasterGamble($userToken, $playType, $sortType, $lvType, $priceType, $unionType, $pageSize, $pageNum, $timestamp, $game_type);
