...
//            $list[$k]['awayTeamLogo']   = getLogoTeam($v['away_team_id'],2,$game_type);
    ...
    public function getLive()
    {
        ...
        if($game_type == 2){
            ...
            //$flashUrl  = 'http://dh.qqty.com/svg/esr-svg/svg-f-animate.html?game_id='.$game_id;
        }
            ...
            foreach ($videoInfo as $k => $v)
            {
                if ($v['appname'] && $v['appurl'])
                {
                    $video[] = [
                        ...
                        'app_ischain'   => $v['app_ischain'] == 2 ? '0' : $v['app_ischain'],
        ...
        //是否有动画直播
        $flashList  = $gameModel->where(['game_id'=>$game_id, 'game_state'=>['IN', $stateArr]])->order('update_time desc')->find();
        $has = $linkModel->field('game_id,is_link,flash_id,md_id')->where(['game_id' => $game_id])->find();
        if($has && $flashList){
        ...
        $this->ajaxReturn(['video'=>$video?:[],'flash'=>$flash?:'','gtime'=> $videoList['gtime']?:'']);
    }
