...
class ApnsController extends PublicController
{
    ...
    public function followGame()
    {
        ...
        //APNS设备号
        if($this->param['pushID'] != '' && (!$cert_no || $cert_no == 'APNS_distribution')){
...
//       }
    ...
    public function getApnsUser(){
        $res2 = M('ApnsUsers')->where(['user_id' => ['IN',explode(',', $this->param['user_id'])]])->select();
        $t = time();
        foreach($res2 as $k=>$v){
            $this->oRedis->rpush('apns_user_gameball_push_queue',json_encode(['device_token' => $v['device_token'], 'content' => '全球体育赛事订阅 '.date('Y-m-d H:i:s', $t),'pub_id' => $v['user_id']]));
        }
        dump($res2);
    }
