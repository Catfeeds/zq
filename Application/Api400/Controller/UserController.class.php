...
class UserController extends CommonController
{
    ...
    public function index()
    {
        $userInfo = M('FrontUser')
                ->field(['nick_name','username','area_code','lv','lv_bet','lv_bk','head face','point','coin','unable_coin','descript','weixin_unionid','sina_unionid','qq_unionid','mm_unionid'])
    ...
    public function tradeLog()
    {
        ...
        if($game_type == 1){
            ...
            $list = M('QuizLog q')->field(['q.cover_id user_id','q.gamble_id','q.game_id'])
                ...
                ->where(['q.user_id'=>$this->userInfo['userid'], 'q.game_type' => $game_type, 'gm.play_type' => $string])
                ->page($page.','.$pageNum)
                ->order('q.id desc')
                ->select();
        }
