...
class VideoController extends PublicController
{
    ...
    public function index()
    {
        ...
        $field = ['game_id','game_date','game_time','union_name','home_team_id','home_team_name','away_team_id','away_team_name','game_state','score','app_video'];
        $list = M('GameFbinfo')->field($field)->where($where)->order('game_date asc,game_time asc')->select();
    ...
    public function videoList()
    {
        $videoList = M('Highlights')->field(['title','remark','img','app_url','app_ischain','is_prospect'])
                    ->where(['game_id'=>$this->param['game_id'],'game_type'=>1])
                    ->order('add_time asc')
                    ->select();
