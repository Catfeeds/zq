...
class EasemobController extends PublicController
{
    ...
    */
    ...
    public function joinChatroom()
    {
        ...
        $gameInfo = M('GameFbinfo')->field(['game_date','game_time','game_state'])->where(['game_id'=>$this->param['game_id']])->find();

        if (!$gameInfo || !in_array($gameInfo['game_state'], [0,1,2,3,4,-1]))
            $this->ajaxReturn(3007);

        //进入聊天室时间在比赛前1小时后
        $beginTime = strtotime($gameInfo['game_date'].$gameInfo['game_time']);
