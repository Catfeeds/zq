...
class PayNotifyController extends Controller
{
    ...
    public function notifyAlipay()
    {
        ...
        logRecord("Alipay交易原始数据：".json_encode($_POST));
    ...
    public function notifyWxpay()
    {
        ...
        $_POST['SERVER_NAME'] = $_SERVER['SERVER_NAME'];
        logRecord("WX交易原始数据：".json_encode($_POST));
        unset($_POST['SERVER_NAME']);
    ...
    public function notifyWabpPay()
    {
        ...
        $_POST['SERVER_NAME'] = $_SERVER['SERVER_NAME'];
        file_put_contents('wabp.txt', "WabpPay交易原始数据：".json_encode($_POST)."\r\n\r\n",FILE_APPEND);
        unset($_POST['SERVER_NAME']);
    ...
    //app安卓移动wabp回调接口
    public function notifyMovePay()
    {
        if(IS_POST)
        {
            $fileContent = file_get_contents('php://input');
            logRecord("move交易原始数据：".$fileContent,'logMove.txt');
            /*$fileContent = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><msgReq><msgType>PayResultNotifyReq</msgType><msgVer>1.0</msgVer><payId>20170118172848009507</payId><orderId>CML201701181728465889172</orderId><payResult>0</payResult><amount>20</amount><payOrganization>ALIPAY</payOrganization><payWay>APP</payWay><payType>1</payType><payDate>20170118172854</payDate><hmac>AF949F884F4BA8440AE27DA9F4D68C25</hmac></msgReq>';*/
            
            libxml_disable_entity_loader(true); //禁止引用外部xml实体
            //先把xml转换为simplexml对象，再把simplexml对象转换成 json，再将 json 转换成数组。
            $postInfo = json_decode(json_encode(simplexml_load_string($fileContent, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
            $payId     = $postInfo['payId'];
            $orderId   = $postInfo['orderId'];    //订单号
            $payResult = $postInfo['payResult'];
            $amount    = $postInfo['amount']/100; //实际支付金额
            $payDate   = $postInfo['payDate'];    //交易时间
            
            //支付结果，0：表示支付成功,1：表示支付失败.2：对账补单支付成功
            if($payResult == 0)
            {
                $trade     = M('TradeRecord');
                $where     = ['trade_no'=>$orderId]; //订单号条件
                $data = [
                    'trade_no'         => $orderId,
                    'alipay_trade_no'  => $payId,
                    'pay_fee'          => $amount,
                    'trade_state'      => 1,
                    'etime'            => strtotime($payDate),
                ];
                //查询订单
                $order = $trade->where($where)->find();
                if(!empty($order))
                {
                    if($order['total_fee'] != $amount)
                    {
                        logRecord("实际充值与订单金额不一致：".$orderId,'logMove.txt');
                        echo 'fail';
                        exit;
                    }

                    $trade->where($where)->save($data); //修改订单成功状态
                    //执行充值
                    if(ExecutiveRecharge($orderId)) 
                    {
                        logRecord("金币添加成功：".$orderId,'logMove.txt');
                        echo 'success';
                    }
                    else
                    {
                        logRecord("金币添加不成功：".$orderId,'logMove.txt');
                        echo 'fail';
                    }
                }
                else
                {
                    logRecord("订单不存在：".$orderId,'logMove.txt');
                    echo 'fail';
                }
            }
            else
            {
                logRecord("订单支付失败：".$orderId,'logMove.txt');
                echo 'fail';
            }
        }
    }
    ...
    //app安卓移动sdk回调接口
    public function notifySdkPay()
    {
        if(IS_POST)
        {
            $fileContent = file_get_contents('php://input');
            logRecord("Sdk交易原始数据：".$fileContent,'logMove.txt');
            /*$fileContent = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><msgReq><msgType>PayResultNotifyReq</msgType><msgVer>1.0</msgVer><payId>20170118172848009507</payId><orderId>CML201701181728465889172</orderId><payResult>0</payResult><amount>20</amount><payOrganization>ALIPAY</payOrganization><payWay>APP</payWay><payType>1</payType><payDate>20170118172854</payDate><hmac>AF949F884F4BA8440AE27DA9F4D68C25</hmac></msgReq>';*/
            ...
            $postInfo = json_decode(json_encode(simplexml_load_string($fileContent, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
            $payId     = 'androidsdk';
            $orderId   = $postInfo['ExData'];    //订单号
            $amount    = $postInfo['Price'];     //实际支付金额
            $ReturnStatus = $postInfo['ReturnStatus'];
            $payDate   = time();    //交易时间
            ...
            if($ReturnStatus == 0)
            {
                $trade     = M('TradeRecord');
                $where     = ['trade_no'=>$orderId]; //订单号条件
                $where['trade_state'] = ['neq',1];
                
                //查询订单
                $order = $trade->field(['total_fee'])->where($where)->find();
                if(!empty($order))
                {
                    $data = [
                        'trade_no'         => $orderId,
    ...
    {
        vendor('Payment.PayPal.paypal_functions');
        ...
        if ( $token != "" )
        {
            ...
            if( $ackGetExpressCheckout == "SUCCESS" || $ackGetExpressCheckout == "SUCESSWITHWARNING") 
            {
                ...
                exit("<script>window.location.href = 'PayPal:ERROR'; </script>");
            } 
            else  
            {
                //查询订单失败
            ...
            {
                exit("<script>window.location.href = 'PayPal:SUCCESS'; </script>");
            }
            else  
            {
                //充值失败
    ...
    public function notifyPayPal()
    {
        ...
        try {
            $trade_no = I('post.custom');
            $order = M('TradeRecord')->where(['trade_no'=>$trade_no])->find();
            if(!$order){
                throw new Exception("没有该订单");
            }
            $pay_fee         = I('post.mc_gross');
            if($pay_fee != $order['total_fee']){
                throw new Exception("充值金额数据异常");
            }
            vendor('Payment.PayPal.PaypalIPN');
            $ipn = new PaypalIPN();
            $ipn->useSandbox();
            $verified = $ipn->verifyIPN(); //查询订单是否成功充值
            if ($verified) 
            {
                $alipay_trade_no = I('post.payer_email');
                $data = [
                    'trade_no'        => $trade_no,
                ...
                M('TradeRecord')->where(['trade_no'=>$trade_no])->save($data);
                if(ExecutiveRecharge($trade_no))
                {
                    logRecord("金币添加成功：".$trade_no,'logPayPal.txt');
                    header("HTTP/1.1 200 OK");
                }else{
                    throw new Exception("金币添加失败");
                }
            }
            else
            {
            ...
            logRecord("金币添加不成功：".$trade_no.'，错误：'.$e->getMessage(),'logPayPal.txt');
        ...
        if(IS_POST){
            ...
            switch ($payType) {
                ...
                case 'paypal':
                    ...
                    $rs = M('TradeRecord')->add($data);
                    if($rs)
                    {
                        $parameters = array(
                            'PAYMENTREQUEST_0_AMT' => $ordtotal_fee,
                            'currencyCodeType'     => 'HKD',
                            'paymentType'          => 'Sale',
                            'custom'               => $out_trade_no
                        );
                        $returnURL = 'http://'.$_SERVER['HTTP_HOST'].'/Api300/PayNotify/PayPal';
                        $cancelURL = $_SERVER['HTTP_REFERER'];
                        $resArray  = CallShortcutExpressCheckout ($parameters, $returnURL, $cancelURL);
