...
*/
    ...
    public function set_pwd() {
        ...
        $verifySign = cookie('verifySign');
        if (!$verify || !$verifySign || $verify['mobile'] != $verifySign['mobile'] || $verify['rank'] != $verifySign['rank']) {
            $this->error('验证码信息已经过时，请重新验证手机！', redirect('find_pwd'));
        }
        if (IS_POST) {
            /* 检测数据 */
            $pwd = I('pwd', '');
...
//                }
                ...
                //检查帐号密码
                $user = D('FrontUser')->login($mobile, $pwd);
                if (0 < $user) {//验证成功
                    //根据第三放登录类型组装对应信息
                    switch ($token['type']) {
                        case 'qq':
                    ...
                    //查看是否已绑定其他第三方登录id
                    $is_bind = M("frontUser")->where(['id' => $user])->getField($sdk_unionid);
                    if ($is_bind) {
                        $this->error("绑定失败，该帐号已被绑定！");
                    }
                    //绑定第三方登录id
                    if (!M("FrontUser")->where(['id' => $user])->save($sdk_array)) {
                        $this->error("绑定失败，请稍后再试！");
                    }
                    //******登录用户*****
                    if (D('FrontUser')->autoLogin($user)) {//登录用户
                        S('loginToken:' . session_id(), NULL);
