...
*/
	...
	use Api500\Services\AppfbService;
...
class AppdataController extends Controller
{
    ...
    public function fbInstant()
    {
        ...
        $fbService = new \Home\Services\AppfbService();
	    $res = S('api_fbInstant_'.$unionId.'_'.$subId.'_'.$key);
	    if (!$res){
		    $res = $fbService->fbInstant($unionId,$subId);
		    S('api_fbInstant_'.$unionId.'_'.$subId.'_'.$key, $res, 600);
	    }
    ...
    {
        $gameId = !empty($_REQUEST['gameId'])? $_REQUEST['gameId'] : null;
	    $platform = !empty($_REQUEST['platform'])? $_REQUEST['platform'] : null;
        if(empty($gameId))
        {
            $data['status']  = 0;
            $data['data'] = null;
        ...
        else
        {
        	if (empty($res) && $platform == 2) {
		        ...
		        $res = [];
		        $res[0] = $defaultHomeImg;
		        $res[1] = $defaultAwayImg;
		        $data['data'] = $res;
	        } else {
		        $data['status']  = 1;
		        $data['data'] = $res;
    ...
    */
    ...
    public function strength()
    {
        ...
        */
        ...
        $appService = new Api510\Services\AppfbService();  //mongodb
