...
class UserController extends CommonController
{
    ...
    public function index()
    {
        $userInfo = M('FrontUser')
                ->field(['nick_name','username','area_code','lv','lv_bet','lv_bk','head face','point','coin','unable_coin','descript','weixin_unionid','sina_unionid','qq_unionid','mm_unionid','is_expert','expert_status','reason'])
    ...
    public function myFollowUser()
    {
        ...
        $list = M('FollowUser f')->field('f.follow_id user_id,u.nick_name,u.lv,u.lv_bk,u.lv_bet,f.sub, u.head face,u.descript')
    ...
    public function accountLog()
    {
        ...
        if($type == 3){
            ...
            foreach($list as $k => $v)
            {
                ...
                if(iosCheck()){
                    ...
                    if (I('platform') == 2) $list[$k]['desc'] = str_replace('金', 'Q', $list[$k]['desc']);
                }
        ...
        foreach ($list as $k2 => $v2)
        {
            ...
            if(iosCheck()){
                ...
                if (I('platform') == 2) $list[$k2]['desc'] = str_replace('金', 'Q', $list[$k2]['desc']);
            }
...
*/
        ...
        if($list) {
            foreach ($list as $k => $v) {
                $userInfo = M('FrontUser')->where(['id' => $v['user_id']])->field('lv,lv_bet,lv_bk,nick_name,head')->find();
    ...
    public function applePurchase()
    {
        ...
        if(empty($this->param['payCoin']))
            $this->ajaxReturn(5001);

        $payCoin = intval($this->param['payCoin']);
        //充值配置
        ...
        $user = M('FrontUser')->master(true)->field('coin, unable_coin')->find($this->userInfo['userid']);
        ...
        if($giveCoin) {
            $rs3 = M('AccountLog')->add([
                ...
                'log_time' => NOW_TIME,
                'change_num' => $giveCoin,
                'total_coin' => $user['coin'] + $user['unable_coin'] + $giveCoin,
                'desc' => '苹果内购赠送',
                'platform' => 2,
                'pay_way' => $this->param['sandbox'] != '1' ? 5 : 6, //5：正式上架后的内购充值，6：沙盒内购充值
                'order_id' => NOW_TIME,
        ...
        $this->ajaxReturn(['result'=>1, 'giveCoin' => (string)$giveCoin]);
    }
    ...
    public function diegoOrder(){
        ...
        $bug_log = M('IntroBuy')->alias('B')
            ->field('B.product_id,P.name,P.total_rate,P.rate_num,P.total_num,P.game_num,P.sale,P.logo,B.list_id,L.pub_time,L.remain_num')
    ...
    public function newIntro(){
        ...
        $list = M('IntroFollow')->master(true)->alias('F')
            ->field('G.list_id,P.id product_id,P.name,P.total_rate,P.rate_num,P.total_num,P.game_num,P.sale,P.logo,G.create_time')
