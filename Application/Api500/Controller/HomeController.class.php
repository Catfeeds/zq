...
class HomeController extends PublicController
{
    ...
    public function index()
    {
        ...
        if($indexUserInfo){
            ...
            foreach ($indexUserInfo as $k => $v) {
                ...
                if(iosCheck()) $indexUserInfo[$k]['nick_name'] = str_replace(C('filterNickname'), C('replaceWord'), $indexUserInfo[$k]['nick_name']);
    ...
    {
        $id = I('article_id');
        $game_type = S('articleGameType'.$id) ?: ($this->param['game_type'] ?: 1);//有缓存读缓存；默认1足球，2篮球
        ...
        if(!$detail = S('articleDetail'.$id.get_client_ip())) {
            ...
            $game_type = $detail['gamebk_id'] ? 2 : 1;
    ...
    public function masterGamble(){
        ...
        $platform = iosCheck() ? $this->param['platform'].'1' : $this->param['platform'];
        //高手推荐
        if(!$masterGamble = S('indexMasterGamble' . MODULE_NAME.$game_type.$platform)){
//            if(iosCheck()){
//                $masterGamble = D('GambleHall')->getGamble($game_type);
//            }else{
                $masterGamble = D('GambleHall')->masterGamble($game_type);
//            }

            S('indexMasterGamble' . MODULE_NAME.$game_type.$platform, json_encode($masterGamble), 60 * 10);
        }
