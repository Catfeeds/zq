...
class NewsController extends CommonController {
    ...
    public function index() {
        ...
        if ($class_id == 10) {
            //热盘
            $list = $_M->alias('pl')->field('pl.id,pl.img,pl.content,pl.click_number,pl.add_time,pl.class_id,fu.head,user_id,nick_name,title,is_original,source,remark')->join('left join qc_front_user fu on pl.user_id=fu.id')
                            ->where($where)
                            ->order($this->_order)
                            ->limit($limit)->select();
            ...
            foreach($list as $key=>$val)
            {
                ...
                if(!empty($val['img'])){
                    $list[$key]['img'] = Tool::imagesReplace($val['img']);
                }else{
                    //获取第一张图片
                    $list[$key]['img']  = Tool::getTextImgUrl(htmlspecialchars_decode($val['content']),false)[0]?:staticDomain('/Public/Images/defalut/newsimg.jpg');
                }
            ...
            //分类
            $list = $_M->alias('pl')->field('pl.id,pl.class_id,img,title,is_original,label,source,remark,content,click_number,add_time')
                            ->where($where)
                            ->order($this->_order)
                            ->limit($limit)->select();
            foreach ($list as &$v) {
