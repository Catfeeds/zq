...
class GuessController extends CommonController {
    protected function _initialize() {
        ...
        $this->assign('sConfig',['url'=>'Sporttery']);
    }
...
//						->join('INNER JOIN '.$querySql.' g2 ON g1.user_id = g2.user_id and g1.create_time = g2.create_time')
		...
		if ($login_user_id > 0)
		{
			...
			$userQuizLog = M('QuizLog')
				...
				->field('cover_id,game_id')
				->select();
			foreach ($gambleList as $key => $value)
			{
				foreach ($userQuizLog as $k => $v)
				{
					if ($value['user_id'] === $v['cover_id'] && $value['game_id'] === $v['game_id'])
					{
    ...
    public function rank()
	{
        ...
        if($page > 1 && IS_POST && IS_AJAX){//只返回前100条
            $this->ajaxReturn(['status'=>1, 'list' => [],
                'my_rank'=> '',
                'm_quiz'=>'',
                'is_login'=>$is_login
            ]);
...
//        }
        ...
        $data = I('data',4,'int');
        $type = I('type',1,'int');
        if(!in_array($data,[1,2,3,4])) $data = 4;
        if(!in_array($type,[1,2])) $type = 4;
...
//                $playtype = $v['play_type'] == '1' ? '让球' : '大小球';
                ...
                $li  =  "<li class=\"list\">".
                        ...
                        "<div class=\"etip\">";
                        if($v['desc'] != ''){
                            $li .=  "<span><img src=\"".staticDomain('/Public/Mobile/images/guess/fenxi.png')."\" alt=\"分析\"></span>";
                        }
                        if(($v['is_trade'] == 1 && $v['tradeCoin'] > 0) || ($v['result'] != 0 && $v['tradeCoin'] > 0)){
                            $li .=  "<span class=\"coins\">{$v['tradeCoin']}</span>";
                        }
                        if(($v['is_trade'] == 1 && $v['tradeCoin'] == 0) || ($v['result'] != 0 && $v['tradeCoin'] == 0)){
                            $li .=  "<span><img src=\"".staticDomain('/Public/Mobile/images/guess/free.png')."\" alt=\"免费\"></span>";
                        }
                $li .=  "</div>".
                        "</div>".
                        ...
                        if($v['is_trade'] == 1 || $v['user_id'] == is_login() || $v['result'] != 0)
                        {
                            switch ($v['result']) {
                                ...
                                case '-10': $class = "cancel";break;
                                case '-11': $class = "pending";break;
                                case '-12': $class = "cut";break;
                                case '-13': $class = "interrupt";break;
                                case '-14': $class = "putoff";break;
                            }
    ...
    public function square(){
        ...
        if(IS_AJAX)
        {
            ...
            foreach ($HostUser as $k => $v) 
            {
                ...
                if($v['curr_victs'] > 0)
                {
                    $li .=  "<em><img src=\"".staticDomain('/Public/Mobile/images/xing.png')."\">{$v['curr_victs']}连胜</em>";
                }
                    $li .=  "</p>".
                            "</div>".
                            "</div>".
