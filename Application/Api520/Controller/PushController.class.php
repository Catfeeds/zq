...
/**
 * 用户推送相关
 * @author hunagzl <496331832@qq.com> 2016.01.21
 */
class PushController extends PublicController
{
    ...
    public function createEasemobUser()
    {
        ...
        //保存设备号到redis
        $this->_oRedis->hmset('ws_' . $this->param['deviceID'], ['userid' => $this->_sUserInfo['userid']]);
        $this->_oRedis->expire('ws_' . $this->param['deviceID'], 3600 * 24);
    ...
    public function createUmengUser()
    {
        ...
        if (!$umengUser) {
            ...
            M('EasemobUser')->where($where)->save(['login_time' => NOW_TIME, 'client_id' => I('clientId')]);
        }
    ...
    public function setPush()
    {
        ...
        if($this->param['platform'] == 3){
            ...
            M('EasemobUser')->where(['username' => $this->_sPushName])->save(['is_push' => $isPush]);

            M('ApnsUsers')->where(['user_id' => $this->_sPushName])->save(['is_push' => $isPush]);

            $this->_oRedis->hset($this->_skeyPre . 'user:' . $this->_sPushName, 'is_push', $isPush);

            $this->_oRedis->hset($this->_skeyPre . 'user:' . $this->param['deviceID'], 'is_push', $isPush);
        }elseif ($this->param['platform'] == 2){
            $this->_oRedis->hset($this->_skeyPre . 'user:' . $this->param['pushID'], 'is_push', $isPush);
            $this->_oRedis->hset($this->_skeyPre . 'user:' . $this->param['deviceID'], 'is_push', $isPush);
            if($this->_sUserInfo['userid']){
                $device_token = M('ApnsUsers')->where(['user_id' => $this->_sUserInfo['userid']])->getField('device_token');
                $this->_oRedis->hset($this->_skeyPre . 'user:' . $device_token, 'is_push', $isPush);
            }
        }
    ...
    {
        if (!$this->param['deviceID'] || !$this->param['gameId'] || !$this->param['gameType'] || !$this->param['playType'])
            $this->ajaxReturn(101);
        ...
        //新增赛程、玩法对应关系
        $this->_oRedis->hset('hash_' . $userFollowKey, $this->param['gameId'], $this->param['playType']);

        //设置过期时间
        if ($this->_oRedis->expire($userFollowKey, 3600 * 24) === false)
            $this->ajaxReturn(3005);

        if ($this->_oRedis->expire('hash_' . $userFollowKey, 3600 * 24) === false)
            $this->ajaxReturn(3005);
    ...
    public function cancelFollowGame()
    {
        ...
        switch($this->param['gameType']){
            case 1:
                $userFollowKey = $this->_sFbUserFollowKey;
                $deviceFollowKey = $this->_sFbDeviceFollowKey;
                $gameFollowKey = $this->_sFbGameFollowKey . $this->param['gameId'];
                break;

            case 2:
                $userFollowKey = $this->_sBkUserFollowKey;
                $deviceFollowKey = $this->_sBKDeviceFollowKey;
                $gameFollowKey = $this->_sBkGameFollowKey . $this->param['gameId'];
                break;
        }
        ...
        //取消赛程、玩法对应关系
        $this->_oRedis->hDel('hash_' . $userFollowKey, $this->param['gameId']);

        //用户登录时，取消关注同时也取消本机关注的对应赛事
        if ($this->_sUserInfo['userid']) {
            $this->_oRedis->srem($deviceFollowKey, $this->param['gameId']);
            $this->_oRedis->hDel('hash_' . $deviceFollowKey, $this->param['gameId']);
        }
    ...
    public function myFollowGame()
    {
        ...
        //取消赛程、玩法对应关系 $fbList = $this->_oRedis->hmGet('hash_' . $userFollowKey, ['game_id', 'play_type']);
        $fbDeviceRes = $this->_oRedis->hGetAll('hash_' . $this->_sFbDeviceFollowKey);
        $res1        = $this->_oRedis->hMset('hash_' . $this->_sFbUserFollowKey, $fbDeviceRes);
        $fbRes       = $this->_oRedis->hGetAll('hash_' . $this->_sFbUserFollowKey);

        $bkDeviceRes = $this->_oRedis->hGetAll('hash_' . $this->_sBKDeviceFollowKey);
        $res2        = $this->_oRedis->hMset('hash_' . $this->_sBkUserFollowKey, $bkDeviceRes);
        $bkRes       = $this->_oRedis->hGetAll('hash_' . $this->_sBkUserFollowKey);
...
//	    $fbBaseRes =  array_column(M('GameFbinfo')->field('game_id')->where($map)->select(), 'game_id');
//	    $bkBaseRes =  array_column(M('GameBkinfo')->field('game_id')->where($map)->select(), "game_id");

        foreach($fbRes as $fk => $fv){
//        	if (!in_array($fk, $fbBaseRes)) {
//        	    continue;
//	        }
...
//	        }
    ...
    public function reportChat()
    {
        ...
        $data = [
            'uid'           =>$this->param['eUser'],
            'username'      =>$this->param['eUser'],
            'nickname'      =>$this->param['nickname'],
            'content'       =>utf8_filter($this->param['content']),
...
//            $this->_oRedis->del($this->_sBKDeviceFollowKey);
    ...
    public function pushTest2()
    {
        ...
        $payload = ['aps' => [
            'alert' => ["body" => '全球体育']],
            'e' =>[
                'em_module' => ['module' => $this->param['module'], 'value' => $this->param['value'], 'url' => $this->param['value']],
                'show_type' => 1
            ]
        ];

        $post_data = [
            'token'     => $this->param['device_token'],
            'platform'  => $this->param['cert_no'],
            'payload'   => json_encode($payload)
        ];
        $res = httpPost(C('push_adress'), $post_data);
        var_dump(C('push_adress'),$res);exit;
    }
    ...
    {
        $redis = connRedis();
        echo $redis->lpush(C('em') . 'user_gameball_push_list', $this->param['uid']); //推荐的用户入列
        echo $redis->lpush(C('um') . 'user_gameball_push_list', $this->param['uid']); //推荐的用户入列,友盟
    }
    ...
    public function checkWord(){
        $this->ajaxReturn(['result' => '1', 'msg' => '内容正常']);
//        //敏感词检测
//        if(!matchFilterWords('FilterWords',$this->param['content'])){
//            $result = '0';//不合法
//            $msg    = '您的信息含有敏感字符，请修改后再发';
//        }else{
//            $result = '1';//合法
//            $msg    = '内容正常';
//
...
////            if($this->_sUserInfo['userid']){
////                $data = [
////                    'user_id' => $this->_sUserInfo['userid'],
////                    'content' => utf8_filter($this->param['content']),
////                    'create_time' => NOW_TIME
////                ];
////
////                M('EasemobChatlist')->add($data);
////            }
//        }
//
//        $this->ajaxReturn(['result' => $result, 'msg' => $msg]);
    }
    ...
    public function getBlockUser(){
        ...
        $where = ['user_type' => 1, 'is_block' => 1,['block_time' => ['GT', $time]],'uid'=>['NEQ','']];
        $users = M('EasemobUser')->field(['uid user_id'])->where($where)->select();
        $this->ajaxReturn(['users' => $users?:[]]);
    }
    ...
    public function test(){
        ...
        $redis->lpush(C('em') . 'user_gameball_push_list', $msgid); //推荐的用户入列
        $redis->lpush(C('um') . 'user_gameball_push_list', $msgid); //推荐的用户入列,友盟
        ...
        $content = "您关注的用户 {$pubUser['nick_name']} 发布推荐啦，{$union_name} {$home_team_name} VS {$away_team_name}, 马上查看";
        echo $redis->hmset(C('um') . 'user_gameball_push_msg:' . $msgid, ['user' => $userid, 'content' => $content,'show_type' => 1]); //生成友盟消息
        echo $redis->hmset(C('em') . 'user_gameball_push_msg:' . $msgid, ['user' => $userid, 'content' => $content,'show_type' => 1]); //生成环信消息
