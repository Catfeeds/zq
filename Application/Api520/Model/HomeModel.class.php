...
//                ->limit(6)
            ...
            if ($liveGame) {
                ...
                foreach ($liveGame as $k => $v) {
                    $gameIdArr[] = (int)$v['gameId'];
                }
                $baseRes = $mongo->select('fb_game',['game_id'=>['$in'=>$gameIdArr],'is_flash'=>1],
                    [
                        'game_id','is_flash'
                    ]
                );
                ...
                foreach ($baseRes as $k => $v) {
                    $mongoGame[$v['game_id']] = $v;
                }

                foreach ($liveGame as $k => &$v) {
                    #是否有视频直播
                    $app_video = $v['app_video'];
                    ...
                    //是否有flash
                    if(isset($mongoGame[$v['game_id']])){
                        $flash[]  = $mongoGame[$v['game_id']]['is_flash'];
                    }
                    $is_flash = $mongoGame[$v['game_id']]['is_flash'];
                    if($has_video == 0 && $is_flash == 0){
                        continue;
                    }
