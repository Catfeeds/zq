...
class UserController extends CommonController
{
    ...
    public function gambleInfo()
    {
        ...
        if (in_array($playType, array(0,1,-1)) && $page)
        {
        ...
        $gambleList = D('GambleHall')->getGambleList($userid, 0, 1); //竞猜记录，默认全部
    ...
    public function tradeLog()
    {
        ...
        $list = M('QuizLog q')->field(['q.cover_id user_id','q.gamble_id'])
                ...
                ->where(['q.user_id'=>$this->userInfo['userid'], 'q.game_type' => 1, 'gm.play_type'=>$playType])
                ->page($page.','.$pageNum)
                ->order('q.id desc')
                ->select();
    ...
    public function userInvitation(){
        ...
        if(!$code){
            $code = M('FrontUser')->getInvitationCode($this->userInfo['userid']);
        ...
        $this->ajaxReturn(['info' => $info, 'code' => $code, 'list' => (array)$list]);
    }
    ...
    public function invitationList(){
        //每天12点更新一次
        if(time() >= strtotime('12:00') || !S('invitationList')){
            $list = M('InviteInfo i')->field('i.user_id, i.total_coin, u.nick_name, u.head as face')
                    ->join(' LEFT JOIN qc_front_user u on i.user_id = u.id ')
                    ->order('i.total_coin desc, i.total_num desc, i.first_num desc, i.second_num desc, i.third_num desc')
                    ->limit(10)->select();

            S('invitationList', json_encode($list), 3600 * 36);//保存36小时
        }else{
            $list = S('invitationList');
        }
