...
class PostController extends PublicController
{
    ...
    public function sendPost()
    {
        try {
            ...
            $addArrs['status'] = 1;
            $addArrs['create_time'] = time();

            $post_id = $postModel->add($addArrs);
            if (!$post_id)
                throw new \Think\Exception('', 6011);
            ...
            if (array_sum($_FILES['pics']['size']) > 0) {
                ...
                if (!$re['status'])
                    throw new \Think\Exception($re['info'], 6010);
    ...
    public function myFollowPosts()
    {
        ...
        if (empty($followId)) $this->ajaxReturn(array());
    ...
    public function detail()
    {
        ...
        if(!$detail = S('postDetail'.$this->param['post_id'].MODULE_NAME)) {
            ...
            $detail = (array)M('CommunityPosts p')->field('p.id, p.user_id, p.base64_title as title, p.base64_content as content, p.img, p.like_num, p.like_user, p.create_time, u.nick_name, u.lv, u.head as face')
        ...
        foreach ($comment as $k => $v) {
            ...
            $startNum--;
            $comment[$k]['content'] = (string)base64_decode($v['content']);
            ...
            foreach ($subComment as $kk => $vv) {
                $subComment[$kk]['content'] = (string)base64_decode($vv['content']);
    ...
    public function reply()
    {
        ...
        //修改帖子回复时间
        M('CommunityPosts')->where(['id' => $this->param['post_id']])->save(['lastreply_time' => NOW_TIME]);
