...
class PayNotifyController extends Controller
{
    ...
    public function notifyAlipay()
    {
        ...
        logRecord("Alipay交易原始数据：".json_encode($_POST),'logAli.txt');
        ...
        {//验证成功
            ...
            switch($trade_status)
            {
                case 'TRADE_SUCCESS':
                    ...
                    $sn_type = explode('_', $out_trade_no);
                    if ($sn_type[1] == 'shop2app') {
                        $order_sn = $sn_type[0];
                        $order = M('order', 'tp_', SP_DB)->where(['order_sn' => $order_sn])->find();
                        M('order', 'tp_', SP_DB)->where(['order_id' => $order['order_id']])->save(['pay_status' => 1]);
                        $action_info = array(
                        ...
                        M('order_action', 'tp_', SP_DB)->add($action_info);
                        echo "success";
                        exit;
                    }
                    $data = [
                    ...
                    if($flag === false)
                    {
                        logRecord("金币添加不成功：".$out_trade_no,'logAli.txt');
                        echo 'fail';
                        exit;
                    }
                    logRecord("交易成功：".json_encode($_POST),'logAli.txt');
                    break;
                case 'WAIT_BUYER_PAY':
                    #交易创建
                    $res = $trade->field('*')->where($where)->find();
            ...
            logRecord("验证失败：".$logs,'logAli.txt');
        }
    }
    ...
    public function notifyWxpay()
    {
        ...
        $_POST['SERVER_NAME'] = $_SERVER['SERVER_NAME'];
        logRecord("WX交易原始数据：".json_encode($_POST),'logWx.txt');
        unset($_POST['SERVER_NAME']);
        ...
        if(isset($xmlArr['return_code']) && $xmlArr['return_code'] == 'SUCCESS')
        {
            ...
            $sn_type = explode('_', $xmlArr['out_trade_no']);
            if ($sn_type[1] == 'shop2app') {
                $order_sn = $sn_type[0];
                $order = M('order', 'tp_', SP_DB)->where(['order_sn' => $order_sn])->find();
                M('order', 'tp_', SP_DB)->where(['order_id' => $order['order_id']])->save(['pay_status' => 1]);
            ...
            if($WxPayApi->notify($this->backCall()) === false)
            {
                ...
                echo $xml;
                //调试用，写文本函数记录程序运行情况是否正常
                logRecord("签名验证失败：".json_encode($xmlArr),'logWx.txt');
                exit;
            }
            ...
            switch($xmlArr['result_code'])
            {
                case 'SUCCESS':
                    ...
                    $flag = ExecutiveRecharge($xmlArr['out_trade_no']);
                    if ($flag === false) {
                        logRecord("金币添加不成功：" . $xmlArr['out_trade_no'], 'logWx.txt');
                        echo $fXml;
                        exit;
                    }

                    echo $sXml;
                    logRecord("交易成功：".json_encode($xmlArr).$xmlStr,'logWx.txt');
                    break;
                case 'FAIL':
                    #交易失败
                    echo $fXml;
                    //调试用，写文本函数记录程序运行情况是否正常
                    logRecord("交易失败：".json_encode($xmlArr).$xmlStr,'logWx.txt');
                    break;
                default:
                    break;
            }
        ...
        else
        {
            ...
            echo $fXml;
            //调试用，写文本函数记录程序运行情况是否正常
            logRecord("通信失败：".json_encode($xmlArr).$xmlStr,'logWx.txt');
        }
    }
    ...
    public function notifyYeepay()
    {
        ...
        $_POST['SERVER_NAME'] = $_SERVER['SERVER_NAME'];
        logRecord( json_encode(getParam()),'logYeepay.txt');
        logRecord( json_encode($return),'logYeepay.txt');
        unset($_POST['SERVER_NAME']);

        if (!$return)
            return;
        $sn_type = explode('_', $return['orderid']);
        if ($sn_type[1] == 'shop2app') {
            $order_sn = $sn_type[0];
            $order = M('order', 'tp_', SP_DB)->where(['order_sn' => $order_sn])->find();
            M('order', 'tp_', SP_DB)->where(['order_id' => $order['order_id']])->save(['pay_status' => 1]);
    ...
    public function notifyWabpPay()
    {
        ...
        $_POST['SERVER_NAME'] = $_SERVER['SERVER_NAME'];
        logRecord( "WabpPay交易原始数据：".json_encode($_POST),'logWabp.txt');
        unset($_POST['SERVER_NAME']);
        ...
        logRecord( $xmlInput,'logWabp.txt');
        logRecord( iconv('GB2312', 'UTF-8', $xmlStr),'logWabp.txt');
        logRecord( json_encode($xmlData),'logWabp.txt');
        ...
        {
            logRecord( '签名校验失败：'.$xmlData['APTransactionID'].' '.$_REQUEST['type'],'logWabp.txt');
            exit('签名校验失败');
        }
        ...
        if ($_REQUEST['type'] == 'confirm')
        {
            ...
            {
                logRecord( 'sin无效：'.$xmlData['APTransactionID'],'logWabp.txt');
                exit('sin无效');
            }
            ...
            logRecord( '确认用户状态返回：'.$retXml.$xmlData['APTransactionID'],'logWabp.txt');
            exit($retXml);
        }
        ...
        if ($_REQUEST['type'] == 'notify')
        {
            ...
            logRecord( '执行订单结果：'.json_encode($excu),'logWabp.txt');
            ...
            if ($save !== false && $excu)
            {
                ...
                logRecord( '返回同步订单结果：'.$retXml,'logWabp.txt');
                exit($retXml);
            }
        }
    }
