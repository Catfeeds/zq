...
class HomeController extends PublicController
{
    ...
    public function index()
    {
        ...
        {
            $update_time = (int)isset($this->param['update_time']) ? (int)$this->param['update_time'] : 0;//当前页面最小id
            $articleList = D('Home')->getArticleList((int)$this->param['page'],10, null, $update_time);
            $this->ajaxReturn(['articleList'=>$articleList]);
        }

        //滚动的banner
        if(!$banner = S('HomeBanner'.$this->param['platform']))
        {
            $banner = Tool::getAdList(1,5,$this->param['platform']) ?: null;
            $banner = D('Home')->getBannerShare($banner);
            S('HomeBanner'.$this->param['platform'], json_encode($banner), 60*5);
        }

        //直播和集锦
        if(!$video = S('HomeVideo')){
            $video = D('Home')->getVideo();
            S('HomeVideo', json_encode($video), 60*5);
        }
        ...
        if(!$hotList = S('HomeHotList'.MODULE_NAME)){
            ...
            S('HomeHotList'.MODULE_NAME, json_encode($hotList), 60*5);
        }

        //热门竞猜
        if(!$match = S('HomeMatch')){
            $match = D('Home')->getMatch();
            S('HomeMatch', json_encode($match), 60*2);
        }

        //名师解盘
        $publishOrder = 'is_channel_push desc,add_time desc,id desc'; //资讯排序

        if(!$famous = S('HomeFamous')){
            ...
            S('HomeFamous', json_encode($famous), 60*5);
        }

        //名师解盘轮播
        if(!$famousArticle = S('HomeFamousArticle')){
            $famousArticle = M('PublishList')->field(['id','title'])->where(['class_id'=>10,'status'=>1])->order($publishOrder)->limit(10)->select();
            S('HomeFamousArticle', json_encode($famousArticle), 60*5);
        }

        //资讯快报轮播
        if(!$newsArticle = S('HomeNewsArticle')){
            $newsArticle = M('PublishList')->field(['id','title']) ->where(['class_id'=>['neq',10],'status'=>1])->order($publishOrder)->limit(10)->select();
            S('HomeNewsArticle', json_encode($newsArticle), 20);
        }

        //资讯快报列表
        if(!$articleList = S('HomeArticleList')){
            $articleList = D('Home')->getArticleList(1,10);
            S('HomeArticleList', json_encode($articleList), 20);
        }
    ...
    public function articleDetail()
    {
        ...
        if(!$detail = S('articleDetail'.$id.MODULE_NAME.get_client_ip())) {
            ...
            S('articleDetail'.$id.MODULE_NAME.get_client_ip(), json_encode($detail), C('newsCacheTime'));
        }
