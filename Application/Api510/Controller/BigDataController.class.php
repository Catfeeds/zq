...
/**
 * 大数据接口
 * @author Hmg<huangmg@qc.mail> 2017.11.20
 */

use Think\Controller;
use Think\Tool\Tool;
use Common\Services\MongodbService;
...
class BigDataController extends PublicController
{
    ...
    public function index()
    {
        ...
        if(!$modelList = S('modelListIndex'.MODULE_NAME)) {
            ...
            foreach ($modelList as $k => $v) {
                $modelList[$k]['img'] = $v['img'] ? Tool::imagesReplace($v['img']) : '';
                if(in_array($v['sign'], ['yujing','tongpei'])){
                    unset($modelList[$k]);
                }
            }
            sort($modelList);
        ...
        if(!$bigData = S('bigDataIndex' . MODULE_NAME)){
            ...
            //每日极限：大小球，连大或连小最高
            $jiXian2 = D('BigData')->getDailyMax(3, 1) ?: D('BigData')->getDailyMax(3, 3);
            $bigData['jiXian2'] = isset($jiXian2[0]) ? $jiXian2[0] : (object)[];
            if(is_array($bigData['jiXian2'])) unset($bigData['jiXian2']['handcp']);
    ...
    public function askTheBall()
    {
        ...
        if(!$classArray = S('askTheBall_class'.MODULE_NAME)){
            ...
            if($classArray){
                foreach ($classArray as $k => $v) {
                    ...
                    if(in_array($v['sign'], ['tongpei1','tongpei2','tongpei3','yujing1','yujing2','yujing3'])){
                        unset($classArray[$k]);
                    }
                }
            }
        ...
        $jiXian = $lengRe = $yingPan = $jingCai1 = $jingCai2 = [];
