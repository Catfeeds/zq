...
class TicketController extends PublicController
{
    ...
    public function exchangeTicket(){
        ...
        if($one){
            $giftPrice = $this->param['platform'] == 2 ? C('giftPrice') : C('giftPrice2');
            $data = [];
            $data['name']        = $one['price'].$giftPrice;
            $data['user_id']     = $userToken['userid'];
    ...
    public function ticketType(){
        ...
        if($type == 1){
            ...
            if($res){
                $data[0]['name']      = str_replace('金', 'Q', $res[0]['name']);
            ...
            if($data) {
                foreach ($data as $k => $v) {
                    $data[$k]['name'] = str_replace('金', 'Q', $v['name']);
                    $data[$k]['isExpire'] = 0;
                    if ($v['num'] > 1) {
