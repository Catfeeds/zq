...
class UserController extends CommonController
{
    ...
    public function index()
    {
        $userInfo = M('FrontUser')
                ->field(['id as lnm','nick_name','username','area_code','lv','lv_bet','lv_bk','head face','point','coin','unable_coin','descript','weixin_unionid','sina_unionid','qq_unionid','mm_unionid','is_expert','expert_status','reason'])
...
//                $list[$k]['curr_victs'] = D('GambleHall')->getWinning($v['user_id'], $game_type, 0, $gamebleType, 30)['curr_victs'];
    ...
    public function applePurchase()
    {
        ...
        //充值金额
        $payCoin = $this->param['payCoin'];
        if(empty($payCoin))
            $this->ajaxReturn(5001);

        //是否开通vip操作
        $isVip   = !empty($this->param['isVip']) ? $this->param['isVip'] : 0; 
        //沙河支付测试
        $sandbox = !empty($this->param['sandbox']) ? $this->param['sandbox'] : 0; 
        //登录判断
        $userId  = $this->userInfo['userid'];
        $result  = ExecutiveIosRecharge($payCoin,$userId,$isVip,$sandbox);
        if($result != 1){
