...
class IntroController extends PublicController
{
    ...
    public function index()
    {
        ...
        $products = M('IntroProducts')->alias('P')
            ->field('P.id,P.class_id,P.name,P.`desc`,P.logo,P.sale,P.game_num,P.rate_num,P.total_rate as totalRate, P.remark, C.background')
        ...
        foreach($products as $k5 => $v5){
            ...
            $products[$k5]['win_rate'] = sprintf('近%s中%s',C('introRateNum'),$v5['rate_num']);
        }
        // if(iosCheck()){
        //     //ios审核改为免费
    ...
    public function categoryDetail(){
        ...
        $products = M('IntroProducts')
            ->field('id,class_id,name, `desc`,logo,sale,game_num,rate_num,total_rate as totalRate,remark')
        ...
        foreach($products as $k2 => $v2){
            ...
            $products[$k2]['win_rate'] = sprintf('近%s中%s',C('introRateNum'),$v2['rate_num']);
        }
    ...
    public function productDetail(){
        ...
        $products = M('introProducts')->field('id,name,desc,logo,logo,sale,total_num,pay_num,game_num,rate_num,total_rate as totalRate,create_time')->where(['id' => $productId])->find();
        if($page <= 1 ){
            if(!$products)
            ...
            $products['win_rate']   = sprintf('近%s中%s',C('introRateNum'),$products['rate_num']);
            $products['newIntro']   = $gamble   ?$gamble:[];

            unset($products['create_time']);
            unset($products['ten_num']);
            unset($products['rate_num']);
            ...
            //获取产品回报率
            $intros  = M('IntroLists')->field('return_rate,total_rate as totalRate,create_time')->where(['status' => 1, 'return_rate' => ['exp','is not null'] ,'product_id' => $productId, 'create_time' => ['LT', $blockTime['beginTime']]])->order('create_time ASC')->select();
            $earlyIntroTime = $intros[0]['create_time'];

            //个推介时间距离最早推介时间的天数以及对应的回报率
            foreach($intros as $kIntro => $vIntro){
                $rateArrs[] = $vIntro['totalRate'] ?:'0';
            ...
            //总数计算
            $listCount = M('IntroLists')->where(['status' => 1, 'product_id' => $productId, 'create_time' => ['LT', $blockTime['beginTime']]])->count();
            $ret_data['listCount'] = $listCount;
        ...
        $limit  = 20 * ($products['game_num']?:1);
        $cacheKey = MODULE_NAME . '_historyIntro_list:' . $productId . $blockTime['endTime'] . $page;
...
//        }
        ...
        $historyIntro = M('IntroLists')->alias('L')
            ->field('L.id,L.is_win,G.union_name,U.union_color,G.home_team_name,G.score,G.away_team_name,G.play_type,G.chose_side,G.odds,G.handcp,G.result,G.gtime,L.pub_time')
            ->join('LEFT JOIN qc_intro_gamble G ON L.id = G.list_id')
            ->join('LEFT JOIN qc_union U ON U.union_id = G.union_id')
            ->where(['L.status' => 1, 'L.product_id' => $productId, 'L.create_time' => ['LT', $blockTime['beginTime']]])
            ->order('L.create_time DESC, G.gtime ASC ')
        ...
        foreach($historyIntro as $key=>$value){
            ...
            $_historyIntro[$value['id']]['lise_id']= $value['id'];
            $_historyIntro[$value['id']]['is_win']= $value['is_win'];
        ...
        foreach($_historyIntro as $key2=>$value2){
            $is_order = '0';
            if($value2['is_win'] == 0){
                $is_order = M('IntroBuy')->where(['user_id' => $userInfo['userid'], 'list_id' => $value2['lise_id']])->find()?'1':'0';
            }
            unset($value2['is_win'],$value2['lise_id']);
            $value2['is_order'] = $is_order;
