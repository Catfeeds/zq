...
class PostController extends PublicController
{
    ...
    public function index()
    {
        ...
        $data = M('CommunityPosts p')
                ...
                ->where(['p.status' => 1])->group('p.id')->order('p.id')
                ->getField('p.id, count(cc.post_id) as num');

        $ids = implode(',', array_keys($data));
        $sql = " UPDATE qc_community_posts SET comment_num = CASE id ";
...
*/
        ...
        switch ($cType) {
            case 1://资讯
                $searchFields = array(
                    ...
                    'P.click_number' => 'click_number',
                    'P.source'       => 'source',
                );

                $map['P.id'] = ['NEQ', ''];
                $results = M('communityCollection')
                    ->alias('C')
                    ->field($searchFields)
            ...
            case 2://帖子
                ...
                if(!empty($results)){
                    foreach ($results as $key => $row) {
                        ...
                        $results[$key]['post_content'] = $filterRes ? $filterRes : $results[$key]['post_content'];
    ...
    public function MyPosts()
    {
        $userInfo   = getUserToken($this->param['userToken']);
        $postModel  = D('CommunityPosts');
        $posts      = $postModel->getMyPosts($userInfo['userid'], $this->param['page']);

        $this->ajaxReturn(['lists' => $posts]);
    }
    ...
    public function myFollowPosts()
    {
        ...
        $postModel  = D('CommunityPosts');
        $posts      = $postModel->getMyFllowPosts($followId, $this->param['page']);
    ...
    public function detail()
    {
        ...
        $detail = (array)M('CommunityPosts p')
            ->field('p.id, p.user_id, p.base64_title as title, p.base64_content as content, p.img, p.create_time,p.like_num,p.like_user, u.nick_name, u.head as face')
        ...
        $detail['is_liked'] = isset($userToken['userid']) && in_array($userToken['userid'], explode(',', $detail['like_user'])) ? 1 : 0;

        $filterRes = preg_replace_callback($filterPatten, function ($m) {
            return isset($m[1]) ? '*' : $m[0];
        }, $detail['content']);

        $detail['content'] = $filterRes ? $filterRes : $detail['content'];

        $page = $this->param['page'] ?: 1;
        $pageNum = 20;
        $fields = ['c.id', 'c.user_id','c.status as u_status', 'u.nick_name', 'u.head as face', 'u.status', 'c.filter_content content ', 'c.like_num', 'c.like_user', 'c.create_time', 'c.status'];
        if ($look_id) {
            //评论的顶级id
            $top_id = M('CommunityComment')->where(['id' => $look_id])->getField('top_id');
            $look_id = ($top_id == 0) ? $look_id : $top_id;
            //显示全部评论
            $comment = M('CommunityComment c')
                        ->field($fields)
                        ->where(['c.post_id' => $this->param['post_id'], 'c.pid' => 0])
                        ->join('LEFT JOIN __FRONT_USER__ u ON u.id = c.user_id')
                        ->where(['u.status' => 1, 'c.status'=> 1])
                        ->order('c.create_time desc')
                        ->select();

            $startNum = count($comment);
            $this->isStop = 1;

//            $startNum = M('CommunityComment c')->where(['c.post_id' => $this->param['post_id'], 'c.pid' => 0, 'c.id' => ['ELT', $look_id + 5]])
//                        ->join('LEFT JOIN __FRONT_USER__ u ON u.id = c.user_id')->count();
        ...
        foreach ($comment as $k => $v) {
            ...
            $subComment = M('CommunityComment c')
                        ->field(['c.id', 'c.user_id', 'c.by_user', 'c.like_num', 'c.filter_content content', 'c.status', 'u.id', 'u.status as u_status'])
                        ->join('LEFT JOIN __FRONT_USER__ u ON u.id = c.user_id')
                        ->where(['post_id' => $this->param['post_id'], 'top_id' => $v['id'], 'u.status' => 1])
            ...
            foreach ($subComment as $kk => $vv) {
                ...
                $filterRes3 = preg_replace_callback($filterPatten, function ($m) {
                    return isset($m[1]) ? '*' : $m[0];
                }, $subComment[$kk]['content']);
                $subComment[$kk]['content'] = $filterRes3 ? $filterRes3 : $subComment[$kk]['content'];
            }

            $comment[$k]['subComment'] = $subComment;
            unset($comment[$k]['like_user']);

            $filterRes2 = preg_replace_callback($filterPatten, function ($m) {
                return isset($m[1]) ? '*' : $m[0];
            }, $comment[$k]['content']);

            $comment[$k]['content'] = $filterRes2 ? $filterRes2 : $comment[$k]['content'];
        }
