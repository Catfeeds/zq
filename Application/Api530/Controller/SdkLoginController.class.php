...
//                $sdk_array  = ['weixin_unionid' => $token['unionid']];
                ...
                $token['access_token'] = $this->param['code'];
                $token['openid'] = !$this->param['openid'] ? 'openid' : $this->param['openid'];
                $sdkInfo = $this->getSdkUserInfo($token);

                $sdk_array  = ['weixin_unionid' => $sdkInfo['unionid']];
                break;
        ...
        if ($userInfo) {//已登录过
            ...
            $loginData = D('FrontUser')->loginData($userInfo['id'], $this->param);
        ...
        } elseif (!$userInfo) {//未执行过第三方平台登录，第三方注册：
...
//            D('FrontUser')->giftBag($userid, $this->param['platform'], 1, 3);
            ...
            $loginData = D('FrontUser')->loginData($userid, $this->param);
...
//                $apiUrl = 'https://graph.qq.com/oauth2.0/me?access_token=ACCESSTOKEN&amp;unionid=1';
            ...
            case 'weixin':
                ...
                if ($data['errcode'])
                    $this->ajaxReturn(1062, '登录失败:'.$data['errmsg'], $data['errmsg']);
