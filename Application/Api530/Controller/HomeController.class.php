    ...
    public function offLinePage(){
        ...
        if(count($livingArr) < 10){//直播小于10，则取直播+回访+离线共10条记录,每个主播只取4条
            ...
            $liveUser = M('liveLog')
                ...
                ->where(['Lg.status' => 1,'Lg.live_status' => 0])
                ->group('Lg.user_id')
                ->order('Lg.start_time DESC, Lg.add_time DESC')
                ->limit(10)
                ->select();
        ...
        foreach($lives as $k => $v){
            ...
            if($v['live_status'] != '-1'){
                ...
                $v['live_url'] = (string) $v['live_url'];
                $v['replay_url'] = (string) $v['replay_url'];
            }

            $lives[$k] = $v;
        }

        $offLiveUser['img'] = $offLiveUser['img'] ? (string)Tool::imagesReplace($offLiveUser['img']) : '';

        $this->ajaxReturn(['liveList' => $lives ?:[], 'liveUserInfo' => $offLiveUser ?:[]]);
    }
