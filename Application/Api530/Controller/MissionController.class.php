...
class MissionController extends CommonController
{
    ...
    public function missionAction(){
        ...
        //获取任务信息
        $missionInfo = M('Mission')->where(['id'=>$this->param['missionId'], 'type'=>1, 'status'=>1])->find();
        if (!$missionInfo)
            $this->ajaxReturn(101);
        ...
        if($result){
            ...
            $point += $addPoint;
            D('Mission')->addPointLog($userid, 14, $addPoint, $point, "您已完成[{$missionInfo['name']}]任务");
    ...
    public function dailySignIn(){
        ...
        $point = M('FrontUser')->where(['id'=>$userid])->getField('point');
        $point += $addPoint;
        D('Mission')->addPointLog($userid, 16, $addPoint, $point, "您已连续签到{$sign_num}天");
    ...
    public function share(){
        $otherid    = $this->param['otherid'];
        $type       = $this->param['type']?: 0;
        $plat       = $this->param['plat'];
        $game_type  = $this->param['game_type']?:1;
        $play_type  = $this->param['play_type'];
        $userid     = $this->userInfo['userid'];

        if(!$otherid || !$plat || !$type) $this->ajaxReturn(101);

        //资讯和推荐(不同玩法)每日只需要记录一次
        $sLog = M('Share')->master(true)->where(['user_id' => $userid, 'otherid' => $otherid, 'type' => $type, 'play_type' => $play_type, 'create_time' => ['between', [$this->beginTime, $this->endTime]]])->count();
        if($sLog) $this->ajaxReturn(['result' => 0]);
        ...
        if($type == 1 && $addPoint){
            ...
            $gambleInfo = $model->master(true)->field("id,home_team_name,away_team_name")->where(['game_id'=>$otherid, 'user_id' => $userid, 'play_type' => $play_type])->find();
            if(!$gambleInfo)
                $this->ajaxReturn(101);

            $home_name  = explode(',',$gambleInfo['home_team_name'])[0];
            $away_name  = explode(',',$gambleInfo['away_team_name'])[0];

            switch ($play_type) {
                case  '1':
                case '-1': $descPlay = $game_type == 1?'足球亚盘':'篮球亚盘';
                    break;
                case  '2':
                case '-2': $descPlay = $game_type == 1?'足球竞彩':'篮球亚盘';
                    break;
            }

            D('Mission')->addPointLog($userid, 17, $addPoint, $point, "您已分享{$descPlay}【{$home_name}VS{$away_name}】", $gambleInfo['id']);
    ...
    public function achievementAction(){
        ...
        //获取成就信息
        $missionInfo = M('Mission')->where(['id'=>$this->param['missionId'], 'type'=>2, 'status'=>1])->find();
        ...
        if($result){
            ...
            $point = M('FrontUser')->where(['id'=>$userid])->getField('point');
            $point += $addPoint;
            D('Mission')->addPointLog($userid, 15, $addPoint, $point, "您已完成[{$missionInfo['name']}]成就");
