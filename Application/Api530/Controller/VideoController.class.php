...
//         $has = D('GambleHall')->getFbLinkbet($game_id);
        ...
        $lives = M('liveLog')
            ->alias('Lg')
            ->field('Lg.user_id, Lg.id, Lu.unique_id, Lg.title, Lg.room_id, Lg.start_time, U.nick_name')
            ->join('LEFT JOIN qc_live_user LU ON LU.user_id = Lg.user_id')
            ->join('LEFT JOIN qc_front_user U ON U.id = Lg.user_id')
            ->where(['Lg.status' => 1, 'LU.status' => 1, 'Lg.live_status' => ['IN', [1, 2]],'Lg.game_id' => $game_id])
            ->order('Lg.start_time DESC')
            ->limit(10)
            ->select();

        foreach($lives as $k => $v){
            $lives[$k]['live_id'] = $v['id'];
