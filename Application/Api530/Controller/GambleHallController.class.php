...
//        }
        ...
        if($gameType ==  '1'){
            ...
            switch($type){
                case 1:
                        $playWhere = [-1, 1];
                        $gambleOdds = D('GambleHall')->getGambleOdds($gameId,1);
                        $data['ypOdds'] = $gambleOdds['data'];
                    break;

                case 2:
                        $playWhere = [-2, 2];
                        $gambleOdds = D('GambleHall')->getGambleOdds($gameId,2);
                        $data['jcOdds'] = $gambleOdds['data'];
                    break;
                default:
                    $this->ajaxReturn(101);
                    break;
        ...
        elseif($gameType == '2')
        {
            $gambleOdds = D('GambleHall')->getGambleOddsBk($gameId);
            $data['ypOdds'] = $gambleOdds['data'];
        ...
        $data['gstate_check'] = $gambleOdds['gstate_check'];
    ...
    public function getTenMaster($userToken, $gameId, $gameType = 1, $gambleType = 1, $playType = 0,$getTotal = false)
    {
        ...
        //获取参与该场赛事竞猜的用户
        $fields = ['g.id gamble_id','g.user_id', 'g.play_type','g.chose_side','g.handcp','g.odds',
            'g.is_impt', 'g.result', 'g.voice','g.is_voice', 'g.voice_time', 'g.tradeCoin','g.desc',
            'g.desc_check', 'g.create_time','f.head face', 'f.nick_name', $lvField,'(g.quiz_number + g.extra_number) as quiz_number'
        ];
...
//                $list[$k]['tenGamble'] = $tenGamble;
        ...
        $slice_list = array_slice($list,0,10);
        $tenCount = [];
    ...
    public function userHomePage()
    {
        ...
        if($page == 1){
            $userInfo                = M('FrontUser')->field(['nick_name','lv','lv_bet','lv_bk','descript','head face','is_expert','vip_time'])->where(['id'=>$user_id])->find();
