...
class HomeModel extends \Think\Model
{
    ...
    public function getArticleList($page,$pageNum,$channel_id=null,$update_time=0)
    {
        ...
        if($update_time){//如果传最小id，则向下取
            ...
            $articleList = M('PublishList')->field(['id','short_title title','remark','img','content','update_time'])
                ->where($where)
...
//            $video[$k]['awayTeamLogo']   = getLogoTeam($v['away_team_id'],2);
    ...
    public function getHotList($date)
    {
        ...
        $arr = M()->query($sql);
        $tenGambleRateArr = array();//排序数组

        foreach ($arr as $k => $v) {
            $tenGamble = D('GambleHall')->getTenGamble($v['user_id'], 1);
            $num = 0;
            foreach($tenGamble as $vv){
                if($vv == 1 || $vv == 0.5){
                    $num++;
                }
            }
            $tenGambleRateArr[] = $num;
        }
        array_multisort($tenGambleRateArr, SORT_DESC, $arr);
