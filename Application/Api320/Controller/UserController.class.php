...
class UserController extends CommonController
{
    ...
    public function myFollowGamble()
    {
        ...
        if ($follows) {
            ...
            foreach ($gambleList as $k => $v) {
                ...
                //是否已经购买查看过
                $userInfo['is_trade'] = M('QuizLog')->where(['game_type' => 1, 'user_id' => $this->userInfo['userid'], 'gamble_id' => $v['gamble_id']])
                    ->getField('id') ? '1' : '0';
    ...
    public function tradeLog()
    {
        ...
        foreach ($list as $k => $v)
        {
            ...
            $list[$k]['curr_victs'] = D('GambleHall')->getWinning($v['user_id'],1,$playType,$gamebleType,30)['curr_victs'];
