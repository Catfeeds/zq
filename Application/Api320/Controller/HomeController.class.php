...
class HomeController extends PublicController
{
    ...
    public function index()
    {
        ...
        //购买体验券入口
        $ticket = M('AdverList')->field('title, url, img, module')->where(['class_id' => 27, 'platform' => $this->param['platform'],'status' => 1, 'add_time' => ['elt', NOW_TIME],  'end_time' => ['egt', NOW_TIME]])->select();
        if($ticket){
        ...
        if($indexUserInfo){
            foreach ($indexUserInfo as $k => $v) {
                //最新的未结算的推荐，不只是当天
                $play_type = $v['gameType'] == 1 ? [-1,1] : [-2,2];
                $one = M('Gamble')->where(['user_id' => $v['user_id'], 'play_type' => ['in', $play_type], 'result' => 0])->order('id desc')->find();
    ...
    public function articleDetail()
    {
        ...
        if(!$detail = S('articleDetail'.$id.get_client_ip())) {
            $detail = M('PublishList')->field(['id', 'title', 'add_time', 'app_time', 'source', 'content', 'label', 'user_id', 'class_id', 'game_id'])->where(['id' => $id])->find();
    ...
    public function picDetail()
    {
        ...
        $detail = M('Gallery')->field(['id','title','like_num','like_user','img_array','capture_url'])->where($where)->find();
    ...
    {
        $userToken = getUserToken($this->param['userToken']);
        if (!$totalArr = S('SuperMaster' . $this->param['platform'] . MODULE_NAME)) {
            $blockTime  = getBlockTime(1, $gamble = true);//获取赛程分割日期的区间时间
            ...
            S('SuperMaster' . $this->param['platform'] . MODULE_NAME, $totalArr, 60*2);
        }

        if($totalArr) {
            foreach ($totalArr as $k => $v) {
                if ($userToken) {
                    $totalArr[$k]['is_trade'] = D('Common')->getTradeLog($v['gamble_id'], $userToken['userid']);//是否已查看购买过
                } else {//无登录则全部没有购买
    ...
    public function countAllFiveGamble(){
        ...
        $userArr = M('FrontUser')->master(true)->field('id, gamble_num, bet_num')->select();
