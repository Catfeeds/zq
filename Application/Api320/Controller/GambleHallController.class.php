...
*/
        ...
        $data['gambleTotal'] = strval($rfTotalNum + $dxTotalNum + $rqTotalNum + $brqTotalNum);
        $data['has'] = (string) $has;
    ...
    public function getTenMaster($userToken, $gameId, $gameType = 1, $gambleType = 1, $playType = 0)
    {
        ...
        if($list){
            ...
            if (isset($userInfo['userid'])){
                $gambleId = (array)M('QuizLog')->where(['user_id' => $userInfo['userid'], 'game_id' => $gameId, 'game_type' => 1])->getField('gamble_id',true);
            }
...
//                }
            ...
            if ($userInfo && $userInfo != -1){
                $gambleId = (array)M('QuizLog')->where(['game_type' => 1, 'user_id' => $userInfo['userid'], 'game_id' => $gameId])->getField('gamble_id',true);
            }
    ...
    public function exchange()
    {
        ...
        //兑换中心的优惠券配置信息
        $ticketConfig = getWebConfig('ticket');
    ...
    public function queryByNick()
    {
        ...
        if(isset($this->param['keyword'])){
            ...
            foreach($data as $k2 => $v2){
                ...
                //最新竞猜
                $new_gamble = D('GambleHall')->getGambleList($v2['user_id'], $playType = 0, $page = 1, $gamble_id = 0, $gambleType = 0);
            ...
            $lv = isset($this->param['lv'])?'lv'.$this->param['lv']:'lv_bet'.$this->param['lv_bet'];
            ...
            if(!$data = S($cache_key)){
                ...
                }elseif(isset($this->param['lv_bet'])){//竞彩等级搜索
                    $gambleType = '2';
                ...
                if($FrontUser){
                    ...
                    list($wBegin,$wEnd) = getRankBlockDate(1, 1);//周
                    ...
                    foreach($chunkUsers as $cKey => $cVal){
                        ...
                        $cRes = M('Gamble')->field('user_id, GROUP_CONCAT(result) as result')->where($wWhere)->group('user_id')->select();
                        $userGamble = array_merge($userGamble, $cRes);
                    }
                    $userWeekGamble = array_column($userGamble, 'result', 'user_id');
                    ...
                    foreach ($FrontUser as $k1 => $v1)
                    {
                        ...
                        $weekPercnet    = (string)getGambleWinrate($wWin, $wHalf, $wTransport, $wDonate);
                        $weekSort[]     = $weekPercnet;
                        $FrontUser[$k1]['weekPercnet']  = $weekPercnet;
                    }
                    ...
                    foreach($data as $k3 => $v3){
                        ...
                        $new_gamble = D('GambleHall')->getGambleList($v3['user_id'], $wh['play_type'], $page = 1, $gamble_id = 0, $gambleType);
                        $tenGamble  = M('gamble')->where($wh)->order("id desc")->limit(10)->getField('result', true);

                        $data[$k3]['currVicts']    = D('GambleHall')->getWinning($v3['user_id'], 1, 0, $gambleType, 100)['curr_victs'] ?: '0';
                        $data[$k3]['tenPercnet']   = (string)countTenGambleRate($tenGamble, $gambleType);
