...
class IndexController extends PublicController
{
    ...
    public function adverList()
    {
        ...
        $adver = @Think\Tool\Tool::getAdList($classId,20,$platform);

        foreach ($adver as $k => $v)
        {
            if($platform == 3){
                $adver[$k]['matchId'] = $game_id ? : '';
            }
            unset($adver[$k]['id']);
        }
