...
class MissionController extends CommonController
{
    ...
    public function getShareData(){
        ...
        $play_type = isset($this->param['play_type']) ? (int)$this->param['play_type'] : 0;//默认0，资讯不需要，推荐一定要，让分：1，大小：-1
        $userid = $this->userInfo['userid'];
        ...
        //判断今天是否已经分享入库，资讯和推荐(不同玩法)每日只需要记录一次
        ...
        if($type == 1 && $addPoint != 0){
            ...
            $gambleInfo = M('Gamble')->master(true)->field("home_team_name,away_team_name")->where(['game_id'=>$this->param['otherid'], 'user_id' => $userid, 'play_type' => $play_type])->find();
            $home_name  = explode(',',$gambleInfo['home_team_name'])[0];
            $away_name  = explode(',',$gambleInfo['away_team_name'])[0];

            switch ($play_type) {
                case  '1':
                case '-1': $descPlay = '足球亚盘';  break;
                case  '2':
                case '-2': $descPlay = '足球竞彩';  break;
            }
