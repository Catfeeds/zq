...
class ApnsController extends PublicController
{
    ...
    public function _initialize(){
        ...
        $this->oRedis->sRem('apns_invalid_token_lists_' . C('apns_env'), $this->param['deviceID']);
    ...
    public function binding(){
        ...
        $res1 = M('ApnsUsers')->master(true)->where(['device_token' => $this->param['deviceID']])->find();
    ...
    public function followGame()
    {
        ...
        $cert_no = M('ApnsUsers')->master()->where(['device_token' => $this->param['deviceID']])->getField('cert_no');
        ...
        if(isset($info['userid'])){
            ...
            if($cert_no && $cert_no != 'APNS_distribution'){//副包
        ...
        if($this->param['HxDeviceID'] != ''){
            $this->oRedis->sadd($_preKey . $this->param['HxDeviceID'], $this->param['gameId']);
            $this->oRedis->sadd($hxDeviceFollowKey, $this->param['gameId']);
            if($cert_no && $cert_no != 'APNS_distribution'){//副包
                $this->oRedis->sadd($gameFollowKey2, $this->param['HxDeviceID']);
                $this->oRedis->expire($gameFollowKey2, 3600 * 24);
            }
            ...
            $this->oRedis->expire($_preKey . $this->param['HxDeviceID'], 3600 * 24);
    ...
    public function cancelFollowGame()
    {
        ...
        $this->oRedis->srem($gameFollowKey, $this->param['deviceID']);
        $this->oRedis->srem($hxDeviceFollowKey, $this->param['HxDeviceID']);
        ...
        $this->oRedis->srem($_preKey . $this->param['HxDeviceID'], $this->param['gameId']);
    ...
    public function myFollowGame()
    {
        ...
        $cert_no = M('ApnsUsers')->where(['device_token' => $this->param['deviceID']])->getField('cert_no');

        if(isset($info['userid'])){
            $fbKey    = 'hash_push_apns_user_fb_follow:' . $info['userid'];
            $bkKey    = 'hash_push_apns_user_bk_follow:' . $info['userid'];
        }elseif($this->param['deviceID'] !='' && !$cert_no ){
            $fbKey    = 'hash_push_apns_user_fb_follow:' . $this->param['deviceID'];
            $bkKey    = 'hash_push_apns_user_bk_follow:' . $this->param['deviceID'];
        }elseif($this->param['HxDeviceID'] != ''){
            $fbKey    = 'hash_push_hx_user_fb_follow:' . $this->param['HxDeviceID'];
            $bkKey    = 'hash_push_hx_user_bk_follow:' . $this->param['HxDeviceID'];
        }
...
//        }
    ...
    public function push_test(){
        ...
        $res = $apns->send(I('device_token'), 12313);
        echo $res?'success':'fail, retry'.'<br/>';
        sleep(2);
        $err = $apns->readErrMsg();
        var_dump($err);
        $apns->close();
