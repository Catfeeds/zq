...
//            $video[$k]['awayTeamLogo'] = getLogoTeam($v['away_team_id'], 2);
    ...
    public function getPlayTypeData($playType, $paramType, $rankDate){
        ...
        if($arr) {
            ...
            $playTypeString = $playType== 1 ? [-1,1] : [-2,2];
            foreach ($arr as $k => $v) {
                ...
                $arr[$k]['today_gamble']  = $todayGambleSort[] = M('Gamble')->where(['user_id' => $v['user_id'], 'play_type' => ['in', $playTypeString], 'create_time' => ['between', [$blockTime['beginTime'], $blockTime['endTime']]]])->getField('id') ? 1 : 0;
