 ...
 * @return Model
 */
function DM($name='', $tablePrefix='',$connection='') {
    if(strpos($name,':')) {
        list($class,$name)    =  explode(':',$name);
    }else{
        $class      =   'Think\\Model';
    }
    return new $class($name,$tablePrefix,$connection,$reConn=true);
}
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(isset($host)) {
        ...
        if(C('APP_SUB_DOMAIN_DEPLOY') ) { // 开启子域名部署
            $domain = $domain=='localhost'?'localhost':$sub_domain.strstr($_SERVER['HTTP_HOST'],'.');
            // '子域名'=>array('模块[/控制器]');
