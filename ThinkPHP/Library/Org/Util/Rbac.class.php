...
class Rbac {
    ...
    static public function AccessDecision($appName=MODULE_NAME) {
        ...
        if(self::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
                ...
                $is_true = 0;
                // if(in_array('table', array_keys($_REQUEST)) && CONTROLLER_NAME != 'Index')
                // {
                    //列表权限
                    if($accessList[strtoupper($appName)][$_SERVER['PATH_INFO']]){
                        $is_true = 1;
                    }
                //}else{
                    //主页和操作权限
                    foreach ($accessList[strtoupper($appName)] as $k => $v) {
                        $arr = explode('/', $k);
                        if (count($arr) > 1)
                        {
                            $accessList[strtoupper($appName)][$arr[0]] = $v;
                            unset($accessList[strtoupper($appName)][$k]);
                        }
                    }
                    if($accessList[strtoupper($appName)][CONTROLLER_NAME]){
                        $is_true = 1;
                    }
                //}
    ...
    static public function getAccessList($authId) {
        // $access = S('access_'.$_SESSION['authId']);
        // if($access)
        // {
        //     return $access;
        // }
        ...
        foreach($apps as $key=>$app) {
            ...
            foreach ($modules as $k => $v) {
                $access[strtoupper($appName)][($v['name'])]   =  ($v['name']);
            }
            // 判断是否存在公共模块的权限
            // $publicAction  = array();
            ...
            //     if(empty($rs))
            //     {
            //         $sql    =   "select id,name from qc_node where level=2 and pid=1 and status=1 and id =".$moduleId;
            //         $rs =   $db->query($sql);
            //     }
            //     foreach ($rs as $a){
            //         $action['index']	 =	 $a['id'];
            //         $action['menuid']	 =	 $moduleId;
            //     }
            //     // 和公共模块的操作权限合并
            //     $action += $publicAction;
        ...
        //S('access_'.$_SESSION['authId'],$access,300);
        return $access;
    }
