...
//------------------------------------------------------用户体系
	 ...
	 */
	public function __construct($options,$more=false) {
	...
	*/
	...
	function _getToken()
	{
		...
		S('easemob_token',$tokenResult['access_token'],3*86400); //token存储3天
	...
	*/
	...
	function postCurl($url,$body,$header,$type="POST"){
		...
		curl_setopt ( $ch, CURLOPT_TIMEOUT,8); // 设置超时限制防止死循环
		...
		if(empty($result)){
			if($this->more){
				return ['status' => 0,'httpCode' => $httpCode, 'msg' => "(em)Curl error number:" . $curlErrNo . " , Curl error details:" . $curlErr];
			}
			return $res;
		} else{
			return $result;
