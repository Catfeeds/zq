...
class Mysql extends Db{
    ...
    public function getFields($tableName) {
        $explodeArr = explode(' ', $tableName);
        if($explodeArr > 1){
            //解决数据表别名导致语句错误
            $tableName = $explodeArr[0];
        }
        //去掉不存在的model数据表
        $explodeName = str_replace(C('DB_PREFIX'), '', $tableName);
        if(in_array($explodeName, ['home','pinyin','common','cover','gamblehall','jwt','pay','robot','special','uploads','bkscore','big_data','base','new_base','community_comments','tp_goods','bk_score'])){
            return array();
        }
