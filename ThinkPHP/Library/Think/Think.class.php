...
class Think {
    ...
    static public function halt($error) {
        ...
        if (APP_DEBUG || IS_CLI) {
            ...
            if(IS_CLI){
                logRecord(iconv('UTF-8','gbk',$e['message']).PHP_EOL.'FILE: '.$e['file'].'('.$e['line'].')'.PHP_EOL.$e['trace'], 'error.txt');
                exit(iconv('UTF-8','gbk',$e['message']).PHP_EOL.'FILE: '.$e['file'].'('.$e['line'].')'.PHP_EOL.$e['trace']);
            }
        } else {
            //APP接口统一返回json
            if(strpos(MODULE_NAME, "Api") === 0){
                $url = SITE_URL.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
                $message = is_array($error) ? $error['message'] : $error;
                logRecord('error:'.$message.', url:'.$url.', data:'.json_encode($_REQUEST), 'error.txt');
