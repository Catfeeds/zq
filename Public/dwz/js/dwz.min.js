...
function initUI(_box) {
    ...
    if ($.fn.xheditor) {
        $("textarea[editor]", $p).each(function() {
            var $this = $(this);
            var eid = $this.attr("editor");
            //创建隐藏域
            $this.after('<input type="hidden" name="editor_id" value="' + eid + '" />');
            $this.xheditor({
                skin: 'vista',
                tools: $this.attr("tools") || 'full',
                upLinkUrl: _APP_ + '/Script/upload/id/' + eid,
                upImgUrl: _APP_ + '/Script/upload/id/' + eid,
                upFlashUrl: _APP_ + '/Script/upload/id/' + eid,
                upMediaUrl: _APP_ + '/Script/upload/id/' + eid,
                localUrlTest: localTest,
                remoteImgSaveUrl: _APP_ + "/Script/remoteupload/id/" + eid
    ...
    if ($.fn.navTabTodo) $("a[target=navTabTodo]", $p).ajaxTodo();
...
var navTab = {
    ...
    _init: function() {
        ...
        this._getTabs().each(function(iTabIndex) {
            ...
            $(this).find(navTab._op.close$).unbind("click").click(function() {
                var _menu = $('#navMenu ul').children('.selected').attr('menu');
                // console.log($('ul li '').length);
                var _arr = Array();
                var num = 0;
                $('.navTab-tab li').not('.main').each(function(){
                    if($(this).attr('fat') == _menu)
                    {
                        num++;
                        _arr[num] = $(this).attr('url');
                ...
                if((_arr[num] == $(this).parent().attr('url')))
                {
                    var _num = 0;
                    $('.navTab-panel div').not("#workindex").each(function(){
                        var _div = $(this);
                        if(_div.attr('divtype') == _menu)
                        {
                            if(_div.attr('style') == 'display: block;')
                            {
                                _num++;
                            }
                        }
                    ...
                    if(_num == 0)
                    {
                        $('.navTab-panel div').each(function(){
                            if($(this).attr('divurl') == _arr[num-1])
                            {
                                $('.navTab-panel').children('div').hide();
    ...
    _scrollTab: function(iLeft, isNext) {
        ...
        var _width = parseInt($(".tabsPageHeaderContent").css('width')) - num * 122;
        //console.log(_width);
        if(_width < 17)
        {
            _width = _width;
    ...
    openTab: function(tabid, url, options) {
        ...
        var _type = $('#navMenu ul').children('.selected').attr('menu');
        if (iOpenIndex >= 0) {
            ...
            this._panelBox.append('<div class="page unitBox" divtype="'+ _type +'" divurl="' + url + '"></div>');
        ...
        this._getTabs().eq(this._currentIndex).attr("fat", _type);
    }
}; (function($) {
    $.fn.extend({
                    ...
                    $("a", this).each(function() {
                        if ($(this).hasClass(op.ajaxClass)) {
                            $(this).click(function(event) {
                                ...
                                if (this.href && !jGroup.attr("loaded")) jGroup.loadUrl(this.href, {},
                                function() {
...
function navTabDialogSearch(form, navTabId) {
    var $form = $(form);
    if (form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
    navTab.reload($form.attr('action'), {
        data: $form.serializeArray(),
