...
$(".checkes").on('click',function(){
    ...
    {
        var _chick_type = _check.parent().children().eq(0).attr("checked");
        if(_chick_type)
        {
            _check.parent().children().eq(0).attr("checked",false);
        }else{
            _check.parent().children().eq(0).attr("checked",true);
        }
    }
    ...
    $("."+checktype+" div").each(function(){
        ...
        if(union_check.children(0).attr('checked') == 'checked')
        {
    ...
    var is_zhuke = _parent.children('td').children('.fl').eq(1).children().attr("checked");
    var _zhuke = '';
    if(is_zhuke == 'checked')
    {
        _zhuke = true;
...
$('#compare').change(function(){
    ...
    $(".shuju tr").each(function(){
        var _tr = $(this);
        if(_tr.attr('type') == 'tr')
        {
            if(_tr.attr('value') == _num)
            {
                _tr.css('display','');
            }else{
                _tr.css('display','none');
