...
$("#ensures").on('click', function () {
    ...
    $(".match-team li").each(function () {
        ...
        if (cbox.is(':checked')) {
            ...
            $(".game_list tr").each(function () {
                if ($(this).attr('game_id') == cbox.attr('union_m')) {
...
$(".pitch").on('click', function () {
    ...
    if (_css == 'none') {
        ...
        $(".game_list tr").each(function () {
            ...
            if ($(this).children().children().is(':checked') && _type == 'now') {
                $(this).css('display', '');
                savelog($(this).attr('g_id'));
            } else if ($(this).children().children().is(':checked') == false && _type == 'now') {
                $(this).css('display', 'none');
            }
        });
    }
...
$(".Colladd").on('click', function () {
    ...
    if (seach) {
        ...
        $(".game_list tr").each(function () {
            ...
            var union_s = false;
            $(this).children('td').children('a').children('.union_s').each(function () {
                if ($(this).html().indexOf(seach) != -1) {
                    union_s = true;
                }
            });
            var home_s = false;
            $(this).children('td').children('a').children('.home_s').each(function () {
                if ($(this).html().indexOf(seach) != -1) {
                    home_s = true;
                }
            });
            var away_s = false;
            $(this).children('td').children('a').children('.away_s').each(function () {
                if ($(this).html().indexOf(seach) != -1) {
                    away_s = true;
                }
            });
...
function languageChange() {
    ...
    switch (language) {
        case '2':
            $('#languageContent').html('繁体');
            $('.language.traditional').css('display', 'inline-block');
            break;
        case '3':
            $('#languageContent').html('EN');
            $('.language.english').css('display', 'inline-block');
            break;
        default:
            $('#languageContent').html('简体');
            $('.language.simplified').css('display', 'inline-block');
    }
