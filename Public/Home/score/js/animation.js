...
$(function () {
    ...
    $("#chatTxt").emoji({
        ...
        icons: [{
            name: "通用表情",
            path: staticDomain+"/Public/Home/score/dist/img/tieba/",
            maxNum: 50,
            file: ".png",
            placeholder: "{alias}",
            alias: {
...
function getOdds() {
    $.ajax({
        ...
        url: '/odds',
        dataType: 'json',
        data: {gameId: gameId, game_state : game_state},
        ...
        success: function (res) {
            if (res.code == 200 && typeof res.data == 'object') {
                ...
                $.each(data, function (k, event) {
                    ...
                    li += '<li><div class="timePoint">' + event.time + '</div><div class="triangle-border"><img src="'+staticDomain+'/Public/Home/score/images/scoreLive/' + eventIcon + '" width="30" height="30">' + eventArr[0] + '-' + eventArr[1] + '</div><span class="spotPoint ' + spot + '"></span></li>';
                });
            ...
            case 2:
                if (typeof msg.data == 'object' && msg.data.constructor == Array) {
                    ...
                    $.each(msg.data.reverse(), function (k, event) {
                        ...
                        li += '<li><div class="timePoint">' + eventArr[0] + '</div><div class="triangle-border"><img src="'+staticDomain+'/Public/Home/score/images/scoreLive/' + eventIcon + '" width="30" height="30">' + eventArr[1] + '-' + eventArr[2] + '</div><span class="spotPoint ' + spot + '"></span></li>';
                    });
                    ...
                    var li = '<li><div class="timePoint">' + eventArr[0] + '</div><div class="triangle-border"><img src="'+staticDomain+'/Public/Home/score/images/scoreLive/' + eventIcon + '" width="30" height="30">' + eventArr[1] + '-' + eventArr[2] + '</div><span class="spotPoint ' + spot + '"></span></li>';
...
function say(){
    if (chatOpen != '1' && $('.gameRoom').is(':visible')) {
        showMsg('不在聊天时间，不能发言', 0, 'error');
        return;
    }
    ...
    if (content.length < 1) {
        ...
        var curTopic = '';
        if($('.gameRoom').is(':visible')){
            curTopic = 'qqty/1_' + gameId + '/chat';
            payload.data.room_type = 1;
        }else{
            payload.data.room_type = 2;
            curTopic = $('.liveTopic').val() + 'chat';
        }

        var jsonStr = JSON.stringify(payload);
        MqInit.publishToTopic(curTopic, jsonStr);
    }
    $('#chatTxt').empty();
    $('#chatTxt').focus();
}
