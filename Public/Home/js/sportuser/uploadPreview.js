var uploadPreview = function (setting) {
    ...
    _self.DefautlSetting = {
        ...
        ErrSizeMsg: "图片文件大小超过限制。请上传小于2M的文件",
        callback: function () {
        }
    };
    ...
    _self.Setting = {
        ...
        ErrSizeMsg: _self.IsNull(setting.ErrMsg) ? _self.DefautlSetting.ErrSizeMsg : setting.ErrSizeMsg,
        ...
        document.getElementById(_self.Setting.UpBtn).onchange = function () {
            if (this.value) {
                if (!RegExp("\.(" + _self.Setting.ImgType.join("|") + ")$", "i").test(this.value.toLowerCase())) {
                    _alert("温馨提示", _self.Setting.ErrMsg);
                    this.value = "";
                    return false;
                }
                if ((this.files[0].size / 1024) > 2048) {
                    _alert("温馨提示", _self.Setting.ErrSizeMsg);
                    return false;
                }
