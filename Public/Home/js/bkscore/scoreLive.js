...
var url_name_union = 'bk_index_union';
$(function () {
    //读取cookie缓存保存赛事
...
function checklog()
{
    ...
    // return true;
    if(fbgameHideTr != null && fbgameHideTr.length > 5){
        var fbgameHideTrArray = fbgameHideTr.split('?');
...
{
    var cookieName = url_name;
    if(type == 1)
    {
        cookieName = url_name_union;
    }
...
$('#ensures').on('click', function () {
    ...
    Cookie.setCookie(url_name,'show?');
    // Cookie.setCookie(url_name_union,'');
...
function gameChange(payload) {
    ...
    // payload = '{"status":"1","data":{"290716":["290716","2","","19","32","15","29","4","3","","","","","0","0","0","","","","","","hehehe<br/>qweqe"]},"msg":""}';
    ...
    $.each(data, function (k, v) {
        ...
        //欧赔变化
        if(v[20] != null && v[20] != '' && v[20] != undefined && v[20].length > 3)
        {
...
{
    if(!path.find('.tdBtn').hasClass('icon-top'))
    {
        var class_path = 'over';
        if(status > 0)
        {
...
$('.tdBtn').on('click',function(){
    ...
    Cookie.setCookie(url_name+'top',idStr);
});
...
function scoreInfoStyle(game_id)
{
    var parent = $('.mytable01').find('.list_teble[g_id='+game_id+']');
    var total = parent.find('.score_header td').length;
    var num = 100/total;
    num = num.toFixed(2);
    parent.find('.table-header .score_header td').each(function(){
        $(this).prop('width',num+'%');
    })
    parent.find('.home_tr .score_info td').each(function(){
        $(this).prop('width',num+'%');
    })
    parent.find('.away_tr .score_info td').each(function(){
        $(this).prop('width',num+'%');
    })
}
...
function topGame()
{
    var gameId = Cookie.getCookie(url_name+'top');
