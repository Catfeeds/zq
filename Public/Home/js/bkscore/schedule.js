$(function () {
    // languageChange();
    if(Cookie.getCookie(cookie_name) != null)
    {
        if(Cookie.getCookie(cookie_name).substr(0,4) != 'none' && Cookie.getCookie(cookie_name).substr(0,4) != 'show')
        {
    ...
    if(Cookie.getCookie(cookie_name_union) == null)
    {
        Cookie.setCookie(cookie_name_union,'');
    }
    checklog();
    topGame();
    languageChange();
...
$(".event").on('click', function () {
    ...
    $('.match-team input[class=userid]:checkbox').each(function () {
...
function checklog()
{
    ...
    var unionHideTr = Cookie.getCookie(cookie_name_union);
    var _typeHide = Cookie.getCookie(cookie_type);
    if(fbgameHideTr == '' && unionHideTr == '' && _typeHide == '')
    {
        return true;
    }
    // return true;
    if(_typeHide != '' && _typeHide != null)
    {

        type_li(_typeHide);
    }else{
        if(unionHideTr.length > 1)
        {
            var unionHideTrrArray = unionHideTr.split('?');
            $('.match-team input').attr('checked',false);
            $.each(unionHideTrrArray,function(k,v){
                $("input[union_m='"+v+"']").prop('checked',true);
            })
        }
        if(fbgameHideTr.length > 5){
            ...
            if(fbgameHideTrArray[0] == 'show')
            {
                ...
                $.each(fbgameHideTrArray,function(k,v){
                    $("div[g_id='"+v+"']").css('display','');
                    if(unionHideTr.length < 1) {
                        $("div[g_id='" + v + "'] td").find('.gameId').attr('checked', true);
                    }
                })
            }else{
                $.each(fbgameHideTrArray,function(k,v){
                    $("div[g_id='"+v+"']").css('display','none');
                })
            }
        }
    }
...
//cookie保存赛事勾选
function savelog(gameid,type)
{
    Cookie.setCookie(cookie_type,'');
    var cookieName = cookie_name;
    if(type == 1)
    {
        cookieName = cookie_name_union;
    }
    var log = Cookie.getCookie(cookieName);
    if(gameid > 0){
        Cookie.setCookie(cookieName,log + gameid + '?');
    }
}
...
$(".pitch").on('click', function () {
    _css = $(this).attr('type');
    var _url = Cookie.getCookie(cookie_name);
    if (_css == 'none') {
        if(_url == '' || _url.substr(0,4) == 'show')
        {
            Cookie.setCookie(cookie_name,'none?');
        }
        $(".mytable01 .list_teble").each(function () {
...
$(".type-li02").on('click', function () {
    ...
    Cookie.setCookie(cookie_name,'');
    Cookie.setCookie(cookie_name_union,'');
    Cookie.setCookie(cookie_type,'is_sport');
});
//滚球选项
$(".type-li03").on('click', function () {
    ...
    Cookie.setCookie(cookie_name,'');
    Cookie.setCookie(cookie_name_union,'');
    Cookie.setCookie(cookie_type,'nba');
});
...
$('.tdBtn').on('click',function(){
    ...
    $('.mytable01 .list_teble').each(function(){
        var _li = $(this);
        if(_li.find('.tdBtn').hasClass('icon-top'))
        {
            tmpId[num] = _li.attr('g_id');
            num = num+1;
        }
    });
    tmpId = unique(tmpId);
    ...
    Cookie.setCookie(cookie_name+'top',idStr);
});
...
function topGame()
{
    var gameId = Cookie.getCookie(cookie_name+'top');
    if(gameId != undefined && gameId != null && gameId != '?')
    {
        var _game = gameId.split('?');
        for(var i = 0;i<_game.length;i++)
        {
            if(_game[i].length > 1)
            {
                var game_id = _game[i];
                $("div[g_id='"+game_id+"']").find('.tdBtn').addClass('icon-top');
                var game = $("div[g_id='"+game_id+"']");
                $('.mytable01').prepend(game.clone(true));
                game.addClass('hidden');
            }
        }
