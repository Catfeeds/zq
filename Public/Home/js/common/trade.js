...
function payment(obj,id,coin){
    ...
    if(coin > 0)
    {
        ...
        if(balance >= coin){
            var modalhtml = 
            '<div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" style="margin-top: 18%;margin-left: 35%;">'+
            ...
            '<div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" style="margin-top: 18%;margin-left: 35%;">'+
                '<div class="modal-content" style="width: 40%;">'+
                    ...
                    '<div class="modal-body" style="padding: 20px 50px;">'+
                        ...
                        '</dl>'+
                        '<div class="btn-con" style="text-align: center; margin-top: 15px;">'+
                            '<button style="width: 100px; border-radius: 3px;" type="button" class="btn btn-orange" onclick="window.open(\'//www.'+DOMAIN+'/UserInfo/charge.html\')">马上去</button> '+
        ...
        success: function(data){
            if(data.status == 1){
                ...
                gDialog.fConfirm("温馨提示",data.info,function(rs){
                    ...
                    window.open("//www."+DOMAIN+"/UserInfo/charge.html");
                });
            }
        }
    });
...
function content(obj,data){
    ...
    if(game['tradeCoin'] > 0) showMsg("已成功支付"+game['tradeCoin']+"金币！",0,'success');
}
...
function statistics(obj,data){
    ...
    //结束
    $(obj).parents('.st_count').html(html);
    //展开和隐藏
