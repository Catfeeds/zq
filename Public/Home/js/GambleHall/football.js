...
MqInit.onMessage(function (topic, message) {
    var data = message;
    if (topic.indexOf('fb/goal') > -1 && topic.indexOf('fb/goalpsw') < 0) {
        //足球全场赔率
        getGameOdds(data);
    }else if (topic.indexOf('/fb/change') > -1) {
        //比分变化
        gameChange(data);
    }
}, ['qqty/api500/fb/#']);
...
function getGameOdds(payload){
    ...
    for(var k in data)
    {
        ...
        if (oldOdds.length != 0)
        {
            var fsw_exp_home  = data[k][1];
            var fsw_exp       = data[k][0];
            var fsw_exp_away  = data[k][2];
            var fsw_ball_home = data[k][7];
            var fsw_ball      = data[k][6];
            var fsw_ball_away = data[k][8];
            if(fsw_exp == '封' || fsw_ball == '封'){
                return false;
            }
