...
$.extend($.validator, {
	...
	defaults: {
		...
		normalClass: "field-default",
		focusClass: "field-warning",
		signClass: "field-msg",/*有这个class表示是提示标签 <label for="UserName" tip="请输入用户名"></label>*/
		errorElement: "label",
		focusInvalid: false,//是否自动获得焦点
		errorContainer: $([]),
		errorLabelContainer: $([]),
		onsubmit: true,
		ignore: ":hidden",
	...
	messages: {
		...
		zipcode:"邮政编码格式错误",
		money:"金额格式错误",
		checkpass:"输入6-15位数字或者字母",
		adminpass:"密码由8-20位大写、小写字母和数字组成",
		bankcard:"银行卡号格式错误"
	},
		...
		init: function() {
			...
			$(this.currentForm)
				.validateDelegate(":text, [type='password'], [type='file'], select, textarea, " +
					"[type='number'], [type='search'] ,[type='telephone'], [type='url'], " +
	...
	classRuleSettings: {
		...
		letter: {letter: true},
		ip: {ip: true},
		mobileOrTel: {mobileOrTel: true},
		mobile: {mobile: true},
		checkpass: {checkpass: true},
		adminpass: {adminpass:true},
	...
	methods: {
		...
		date: function( value, element ) {
			...
			return this.optional(element) || !/Invalid|NaN/.test(new Date(value).toString()) || /^\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}[ ]\d{1,2}[:]\d{1,2}[:]\d{1,2}$/.test(value) || /^\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}[ ]\d{1,2}[:]\d{1,2}$/.test(value);
		},
		...
		mobile: function( value, element ) {
			return this.optional(element) || /^1[3456789]{1}\d{9}$/.test(value);
		},

		// 自定义-dengwj-2015-12-9
		// checkpass: function( value, element ) {
		// 	return this.optional(element) || /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,15}$/.test(value);
		// },

		// 自定义-dengwj-2018-06-06
		checkpass: function( value, element ) {
			return this.optional(element) || /^[0-9A-Za-z]{6,15}$/.test(value);
		},

		// 自定义-dengwj-2015-12-9
		adminpass: function( value, element ) {
			return this.optional(element) || /^(?=.*[0-9].*)(?=.*[A-Z].*)(?=.*[a-z].*).{8,20}$/.test(value);
		},

		// 自定义-dengwj-2017-08-18
		bankcard: function( value, element ) {
			return this.optional(element) ||  /^(\d{16,19})$/.test(value);
		},
		
		// mobileOrTel验证手机和电话的，暂时不写
		mobileOrTel: function( value, element ) {
		...
		qq: function( value, element ) {
			return this.optional(element) || /^[1-9]\d{4,13}$/.test(value);
		},
		...
		money: function( value, element ) {
			return this.optional(element) || /^([1-9][\d]{0,7}|0)(\.[\d]{1,6})?$/.test(value);
		},
