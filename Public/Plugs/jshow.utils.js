...
(function($,w){
	...
	$.extend(utils,{
	            ...
	            jQuery.ajax({
	    			...
	    			success: function(data, ts) {
						...
						if(data['result']) {
								...
								if (config.childName){
									var ifr = document.getElementById(config.childName);
									var win = ifr.window || ifr.contentWindow;
									eval("win."+config.childJs);
								}
								if (config.childName2){
									var ifr2 = document.getElementById(config.childName2);
									var win2 = ifr2.window || ifr2.contentWindow;
									eval("win2."+config.childJs2);
								}return;
							...
							message_box.show(sMessage,'danger');
		 ...
		 *    弹出自定义框 可以进行制定URL读取     
		 **/
		fOpenDialog:function(title, url, width) {
			var config = {};
			config.title = title;
			config.url = url;
			config.width = width;
			gDialog.fCreate(config);
		},
	});
})(jQuery,window);
 ...
 window.message_box = (function($) {
     ...
     that.init = function(options) {
     	...
     	var html='<div id="message_box_info" class="alert alert-info alert-block" style="display:none;width:400px;max-widow:40%;position:fixed;'+
     	'left:0px;right:0px;margin-left:auto;margin-right:auto;top:30%;z-index:10000;" role="alert"><button type="button" class="close">&times;</button>'+
     	'<span></span></div>';
     	$("body").append(html);
