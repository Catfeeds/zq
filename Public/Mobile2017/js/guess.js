        ...
        if (data.rankTypeName == undefined || data.rankTypeName == '')//当不是做胜率、盈利筛选操作时，重新加载选择框
        {
            $('#ypTable table').append('<tr bgcolor="#fff">' +
                    ' <td colspan="9" class="f-td">' +
                        ...
                        ' </nav>' +
                        ' <!-- 胜率榜 盈率榜 s-->' +
                        ' <div class="fl fs22 red update_time">每天12:45更新榜单</div>' +
                ...
                if(data.status==1)
                {
                    ...
                    if (list != null) {
                        ...
                        $.each(list, function (k, v) {
                            ...
                            var urlHtml = '//'+DOMAIN+'/Guess/other_page/user_id/'+v['user_id']+'/type/1.html';
                            var html = '<tr class="js-list ios_touch '+modClass+' '+hiddenClass+' " data-id="'+v['user_id']+'">' +
                ...
                if(data.status==1)
                {
                    ...
                    if (list != null) {
                        ...
                        $.each(list, function (k, v) {
                            ...
                            var urlHtml = '//'+DOMAIN+'/Guess/other_page/user_id/'+v['user_id']+'/type/2.html';
                            var html = '<tr class="js-list ios_touch '+modClass+' '+hiddenClass+' " data-id="'+v['user_id']+'">' +
